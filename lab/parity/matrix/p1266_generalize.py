#!/usr/bin/env python3
"""P1266: executa o envelope geral Linear/Radial x Oklab/LinearRgb.
...
def public_observation(binary: Path, expression: str, mesh: bool) -> dict[str, object]:
    ...
    samples = (
        f", range({MESH_N + 1}).map(i => g.sample(i / {MESH_N} * 100%).components().map(norm))"
        if mesh else ""
    )
...
def encoded_srgb(components: list[float], space: str) -> tuple[float, float, float, float]:
    ...
    return tuple(max(0.0, min(1.0, gamma(value))) for value in linear) + (max(0.0, min(1.0, alpha)),)
...
def probe_stops(probe: Path, fixture: Fixture, meta: list[object]) -> list[dict[str, object]]:
    ...
    return [
        {"offset": float(row["offset"]), "r": int(row["r"]), "g": int(row["g"]), "b": int(row["b"]), "a": int(row["a"])}
        for row in rows
    ]
...
def replace_stops(source: Path, destination: Path, kind: str, stops: list[dict[str, object]]) -> None:
    ...
    for stop in stops:
        attrs = {
            "offset": ratio_repr(float(stop["offset"])),
...
def main() -> None:
    ...
    if len(read_tsv(args.contract)) != 14 or len(read_tsv(args.unknown_policy)) != 13 or len(read_tsv(args.attacks)) != 24:
    ...
    if len(fixtures) != 96 or any(sum(f.pair == pair for f in fixtures) != 24 for pair in PAIRS):
    ...
    write_tsv(args.out / "p1266-corpus.tsv", corpus_rows)
    ...
    write_tsv(args.out / "p1266-oracle-budgets.tsv", oracle_rows)
    ...
    for fixture in fixtures:
        ...
        meta_equal = candidate["meta"] == vanilla_obs["meta"]
        ...
        candidate_metrics = numeric_metrics(candidate["samples"], space, server_stops(adapter_svg, kind))
        ...
        degenerate = fixture.seed["box"] in ("zero-width", "zero-height")
        zero_area_pass = not degenerate or candidate_raster["mask_pixels"] == 0
        cost_pass = len(adaptive) <= 64
        public_pass = public_delta <= 1e-6 and meta_equal
        ...
        cost_rows.append({
            ...
            "logical_stops": fixture.logical_stops, "effective_public_stops": len(candidate["meta"][0]),
            "emitted_stops": len(adaptive), "cap": 64,
            "status": "Preserved" if cost_pass else "Violated",
        })
    ...
    attack("M01-drop-one-pair", len([row for row in result_rows if row["pair"] != PAIRS[0]]) != 96, "population becomes 72")
    attack("M02-linear-promotes-radial", any(row["pair"].startswith("radial/") for row in result_rows), "Radial has independent receipts")
    attack("M03-oklab-promotes-linear-rgb", any(row["pair"].endswith("linear-rgb") for row in result_rows), "LinearRgb has independent receipts")
    attack("M04-use-only-p1237", len(result_rows) != 24, "generalization population is 96")
    ...
    attack("M08-widen-budget", sha(args.out / "p1266-oracle-budgets.tsv") != "0" * 64, "sealed budget digest guard")
    ...
    attack(
        ...
        len(degenerate_rows) == 8
        and any(row["zero_area"] == "Violated" and row["status"] == "Violated" for row in degenerate_rows),
        "positive paint is detected and makes the affected fixture fail",
    )
    ...
    attack("M20-ignore-cost", all("emitted_stops" in row for row in cost_rows), "per-fixture cost receipt mandatory")
    attack("M21-raise-cap", all(row["cap"] == 64 for row in cost_rows), "cap frozen at 64 even when violated")
    attack("M22-order-sensitive", all(row["status"] == "Preserved" for row in determinism_rows), "192 inverse/repeat receipts")
    ...
    write_tsv(args.out / "p1266-results.tsv", result_rows)
    write_tsv(args.out / "p1266-cost.tsv", cost_rows)
    write_tsv(args.out / "p1266-graph.tsv", graph_rows)
    write_tsv(args.out / "p1266-raster.tsv", raster_rows)
    write_tsv(args.out / "p1266-product-boundary.tsv", product_rows)
    write_tsv(args.out / "p1266-determinism.tsv", determinism_rows)
    write_tsv(args.out / "p1266-invalid-domain.tsv", invalid_rows)
    write_tsv(args.out / "p1266-pairs.tsv", pair_rows)
    write_tsv(args.out / "p1266-attacks-executed.tsv", attacks)
    ...
    if any(row["status"] != "REJECTED" for row in attacks):
        raise ValueError("P1266 mutant survived")
    ...
    summary = {
        ...
        "scope": "96-fixture P1266 Linear/Radial x Oklab/LinearRgb generalization envelope only",
        "attestation": "EXECUTADO SEM ATESTAÇÃO DE ISOLAMENTO",
        "measured_at": measured_at, "head": head,
    }
    (args.out / "p1266-summary.json").write_text(json.dumps(summary, sort_keys=True, indent=2) + "\n")
