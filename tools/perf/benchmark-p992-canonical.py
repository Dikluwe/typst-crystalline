#!/usr/bin/env python3
"""Benchmark P992 — `limits()`/`scripts()` reconhecidas no eval math
(`Content::MathLimitsOverride`, `layout_attach`).

"antes" = binário release do estado pós-P991 (`b59db7f16`), copiado para
temp/p992/typst-antes antes do rebuild. "depois" = release actual (working
tree com P992).
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p992-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p992" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p992] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p992"] / means["antes"]
        rows.append((name, means["antes"], means["p992"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p992']:.2f}ms ratio={ratio:.3f}")
