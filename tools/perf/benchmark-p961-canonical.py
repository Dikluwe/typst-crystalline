#!/usr/bin/env python3
"""Benchmark P961 — legenda de underbrace em tamanho de script + itálico em base de acento/underover.

"antes" = binário release do estado pós-P957 (pré-P961), copiado para
temp/p961/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P961 (underover.rs + math/layout/mod.rs) sobre `907be103f`
(ver relatório P961).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p961-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p961" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p961] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p961"] / means["antes"]
        rows.append((name, means["antes"], means["p961"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p961']:.2f}ms ratio={ratio:.3f}")
