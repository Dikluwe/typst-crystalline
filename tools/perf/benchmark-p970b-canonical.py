#!/usr/bin/env python3
"""Benchmark P970b — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P970b), copiado para
temp/p970b/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P970b (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P970b).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p970b-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p970b" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p970b] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p970b"] / means["antes"]
        rows.append((name, means["antes"], means["p970b"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p970b']:.2f}ms ratio={ratio:.3f}")
