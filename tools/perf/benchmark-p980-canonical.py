#!/usr/bin/env python3
"""Benchmark P980 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P980), copiado para
temp/p980/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P980 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P980).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p980-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p980" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p980] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p980"] / means["antes"]
        rows.append((name, means["antes"], means["p980"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p980']:.2f}ms ratio={ratio:.3f}")
