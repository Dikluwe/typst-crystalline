#!/usr/bin/env python3
"""Benchmark P968 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P968), copiado para
temp/p968/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P968 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P968).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p968-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p968" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p968] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p968"] / means["antes"]
        rows.append((name, means["antes"], means["p968"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p968']:.2f}ms ratio={ratio:.3f}")
