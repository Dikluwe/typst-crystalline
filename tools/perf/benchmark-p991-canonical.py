#!/usr/bin/env python3
"""Benchmark P991 — `layout_grid` sem `&`: `GridAlign::Center` em vez de
`Alternating` (mod.rs::layout_grid).

"antes" = binário release do estado pós-P990 (`e0e8ca4dc`), copiado para
temp/p991/typst-antes antes do rebuild. "depois" = release actual (working
tree com P991).
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p991-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p991" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p991] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p991"] / means["antes"]
        rows.append((name, means["antes"], means["p991"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p991']:.2f}ms ratio={ratio:.3f}")
