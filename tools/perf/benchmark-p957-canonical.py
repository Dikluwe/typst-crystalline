#!/usr/bin/env python3
"""Benchmark P957 — impacto da correcção do posicionamento de peças de assembly.

"antes" = binário release do estado pós-P956 (pré-P957), copiado para
temp/p957/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com a correcção de `assembly.rs` sobre `00cd5bc56` (ver relatório P957).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p957-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p957" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p957] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p957"] / means["antes"]
        rows.append((name, means["antes"], means["p957"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p957']:.2f}ms ratio={ratio:.3f}")
