#!/usr/bin/env python3
"""Benchmark P962 — dif/Dif com wrapper upright (d reto do diferencial).

"antes" = binário release do estado pós-P957 (pré-P962), copiado para
temp/p962/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P962 (structural.rs) sobre `24aecc438`
(ver relatório P962).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p962-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p962" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p962] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p962"] / means["antes"]
        rows.append((name, means["antes"], means["p962"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p962']:.2f}ms ratio={ratio:.3f}")
