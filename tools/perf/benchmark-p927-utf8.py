#!/usr/bin/env python3
"""Benchmark P927 — casos UTF-8 (05-utf8 + blocos Unicode isolados).

Compara o binário cristalino original (target-original/release/typst, commit
`da18ea9f3`) contra o protótipo da Opção 6 (target/release/typst, scan
condicional no source bruto).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p927-utf8"
...
def run(cmd, **kwargs):
    print(f"[p927-utf8] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc, capture_timings in docs:
        if not doc.exists():
            print(f"[p927-utf8] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p927-utf8] {name}", flush=True)
        ...
        print(f"[p927-utf8] {name}: original={original_ms:.2f}ms proto={proto_ms:.2f}ms ratio={ratio:.2f}x")
    ...
    print(f"\n[p927-utf8] Attestation escrita em {attestation_path}")
