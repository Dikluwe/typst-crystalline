#!/usr/bin/env python3
"""Benchmark P941 — coverage exacta lazy com mmap partilhado pelo fontdb.

Compara o binário cristalino P941 contra:
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p941"
...
BINS = {
    "p941": ROOT / "target" / "release" / "typst-p941",
    "p940": ROOT / "target" / "release" / "typst-p940",
...
def run(cmd, **kwargs):
    print(f"[p941] {' '.join(str(c) for c in cmd)}")
...
def run_hyperfine(name: str, doc: Path):
    ...
    commands = [
        f"{BINS['p941']} {doc} /dev/null",
        f"{BINS['p938']} {doc} /dev/null",
        f"{BINS['p940']} {doc} /dev/null",
...
def main():
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p941] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p941] {name}", flush=True)
        hf = run_hyperfine(name, doc)
        p941_ms = hf.get("p941", 0.0)
        p938_ms = hf.get("p938", 0.0)
        p940_ms = hf.get("p940", 0.0)
        ...
        rows.append({
            ...
            "ratio_p941_p938": p941_ms / p938_ms if p938_ms > 0 else float("inf"),
            "ratio_p941_p940": p941_ms / p940_ms if p940_ms > 0 else float("inf"),
            "ratio_p941_vanilla": p941_ms / vanilla_ms if vanilla_ms > 0 else float("inf"),
        })
        print(
            f"[p941] {name}: p941={p941_ms:.2f}ms p938={p938_ms:.2f}ms "
            f"p940={p940_ms:.2f}ms vanilla={vanilla_ms:.2f}ms"
        )
    ...
    print(f"\n[p941] Attestation escrita em {attestation_path}")
