#!/usr/bin/env python3
"""Benchmark P928 — casos UTF-8 (05-utf8 + blocos Unicode isolados).
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p928-utf8"
...
def run(cmd, **kwargs):
    print(f"[p928-utf8] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc, capture_timings in docs:
        if not doc.exists():
            print(f"[p928-utf8] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p928-utf8] {name}", flush=True)
        ...
        print(f"[p928-utf8] {name}: baseline={original_ms:.2f}ms p928={proto_ms:.2f}ms ratio={ratio:.2f}x")

        if capture_timings:
            timings["baseline"] = run_timings(doc, ORIGINAL_BIN, RESULTS_DIR / f"{name}-baseline-timings.json")
            timings["p928"] = run_timings(doc, PROTO_BIN, RESULTS_DIR / f"{name}-p928-timings.json")
    ...
    print(f"\n[p928-utf8] Attestation escrita em {attestation_path}")
