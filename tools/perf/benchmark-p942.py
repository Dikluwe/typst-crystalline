#!/usr/bin/env python3
"""Benchmark P942 — coverage exacta lazy com mmap partilhado pelo fontdb.

Compara o binário cristalino P942 contra:
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p942"
...
BINS = {
    "p942": ROOT / "target" / "release" / "typst-p942",
...
def run(cmd, **kwargs):
    print(f"[p942] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p942] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p942] {name}", flush=True)
        hf = run_hyperfine(name, doc)
        p942_ms = hf.get("p942", 0.0)
        ...
        rows.append({
            ...
            "ratio_p942_p938": p942_ms / p938_ms if p938_ms > 0 else float("inf"),
            "ratio_p942_p940": p942_ms / p940_ms if p940_ms > 0 else float("inf"),
            "ratio_p942_vanilla": p942_ms / vanilla_ms if vanilla_ms > 0 else float("inf"),
        })
        print(
            f"[p942] {name}: p942={p942_ms:.2f}ms p938={p938_ms:.2f}ms "
    ...
    print(f"\n[p942] Attestation escrita em {attestation_path}")
