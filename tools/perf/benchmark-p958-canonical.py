#!/usr/bin/env python3
"""Benchmark P958 — resolução de símbolos gregos no fallback de FuncCall + 13 nomes na tabela.

"antes" = binário release do estado pós-P957 (pré-P958), copiado para
temp/p958/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P958 (symbols.rs + eval/math.rs) sobre `d472b523a`
(ver relatório P958).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p958-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p958" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p958] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p958"] / means["antes"]
        rows.append((name, means["antes"], means["p958"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p958']:.2f}ms ratio={ratio:.3f}")
