#!/usr/bin/env python3
"""Benchmark P981 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P981), copiado para
temp/p981/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P981 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P981).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p981-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p981" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p981] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p981"] / means["antes"]
        rows.append((name, means["antes"], means["p981"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p981']:.2f}ms ratio={ratio:.3f}")
