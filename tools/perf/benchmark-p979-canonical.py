#!/usr/bin/env python3
"""Benchmark P979 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P979), copiado para
temp/p979/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P979 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P979).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p979-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p979" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p979] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p979"] / means["antes"]
        rows.append((name, means["antes"], means["p979"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p979']:.2f}ms ratio={ratio:.3f}")
