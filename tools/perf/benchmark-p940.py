#!/usr/bin/env python3
"""Benchmark P940 — coverage exacta lazy com mmap partilhado pelo fontdb.

Compara o binário cristalino P940 contra:
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p940"
...
def run(cmd, **kwargs):
    print(f"[p940] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p940] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p940] {name}", flush=True)
        ...
        rows.append({
            ...
            "ratio_p940_p938": p940_ms / p938_ms if p938_ms > 0 else float("inf"),
            "ratio_p940_p937": p940_ms / p937_ms if p937_ms > 0 else float("inf"),
            "ratio_p940_vanilla": p940_ms / vanilla_ms if vanilla_ms > 0 else float("inf"),
        })
        print(
            f"[p940] {name}: p940={p940_ms:.2f}ms p938={p938_ms:.2f}ms "
    ...
    print(f"\n[p940] Attestation escrita em {attestation_path}")
