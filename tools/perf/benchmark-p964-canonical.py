#!/usr/bin/env python3
"""Benchmark P964 — variantes gregas de símbolo + ∂ no plano itálico math.

"antes" = binário release do estado pós-P957 (pré-P964), copiado para
temp/p964/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P964 (math_style.rs) sobre `acacf036d`
(ver relatório P964).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p964-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p964" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p964] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p964"] / means["antes"]
        rows.append((name, means["antes"], means["p964"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p964']:.2f}ms ratio={ratio:.3f}")
