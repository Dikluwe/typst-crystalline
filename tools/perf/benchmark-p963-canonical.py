#!/usr/bin/env python3
"""Benchmark P963 — is_text_like exclui bases esticadas (drop terms de scripts laterais).

"antes" = binário release do estado pós-P957 (pré-P963), copiado para
temp/p963/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P963 (attach.rs) sobre `eb080abc4`
(ver relatório P963).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p963-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p963" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p963] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p963"] / means["antes"]
        rows.append((name, means["antes"], means["p963"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p963']:.2f}ms ratio={ratio:.3f}")
