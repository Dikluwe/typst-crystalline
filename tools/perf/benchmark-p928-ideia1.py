#!/usr/bin/env python3
"""Benchmark P928 Ideia 1 — scan paralelo de coverage.

Compara o binário com P927 (scan sequencial) contra o protótipo com
`candidates_for_char` paralelo por slots.
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p928-ideia1"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p923"
ORIGINAL_BIN = ROOT / "target" / "release" / "typst-p927"
...
def run(cmd, **kwargs):
    print(f"[p928-ideia1] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc, capture_timings in docs:
        if not doc.exists():
            print(f"[p928-ideia1] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p928-ideia1] {name}", flush=True)
        ...
        print(f"[p928-ideia1] {name}: original={original_ms:.2f}ms proto={proto_ms:.2f}ms ratio={ratio:.2f}x")
    ...
    print(f"\n[p928-ideia1] Attestation escrita em {attestation_path}")
