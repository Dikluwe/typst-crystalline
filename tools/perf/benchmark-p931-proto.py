...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p931-proto"
...
def run(cmd, **kwargs):
    print(f"[p931-proto] {' '.join(str(c) for c in cmd)}")
...
def benchmark_suite(suite_name: str, docs, corpus_dir: Path, warmup: int, min_runs: int):
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p931-proto] SKIP {suite_name}/{name}: ficheiro não encontrado: {doc}")
            continue
        out = RESULTS_DIR / suite_name / f"{name}.json"
        print(f"[p931-proto] {suite_name}/{name}", flush=True)
        hf = run_hyperfine(name, doc, out, warmup, min_runs)
        p927_ms = hf.get("p927", 0.0)
        proto_ms = hf.get("p931-proto", 0.0)
        ratio = proto_ms / p927_ms if p927_ms > 0 else float("inf")
        rows.append({
            "name": name,
            "p927_ms": p927_ms,
            "p931_proto_ms": proto_ms,
            "ratio": ratio,
        })
        print(f"[p931-proto] {suite_name}/{name}: p927={p927_ms:.2f}ms p931-proto={proto_ms:.2f}ms ratio={ratio:.2f}x")
    return rows
...
def main():
    ...
    attestation = {
        ...
        "binaries": {
            ...
            "p931_proto": {"path": str(PROTO_BIN), "sha256": sha256_file(PROTO_BIN)},
        },
        "canonical": canonical,
        "utf8": utf8,
    }
    ...
    print(f"\n[p931-proto] Attestation escrita em {attestation_path}")
