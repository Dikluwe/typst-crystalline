#!/usr/bin/env python3
"""Benchmark P989 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P989), copiado para
temp/p989/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P989 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P989).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p989-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p989" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p989] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p989"] / means["antes"]
        rows.append((name, means["antes"], means["p989"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p989']:.2f}ms ratio={ratio:.3f}")
