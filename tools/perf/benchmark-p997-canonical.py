#!/usr/bin/env python3
"""Benchmark P997 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P997), copiado para
temp/p997/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P997 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P997).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p997-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p997" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p997] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p997"] / means["antes"]
        rows.append((name, means["antes"], means["p997"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p997']:.2f}ms ratio={ratio:.3f}")
