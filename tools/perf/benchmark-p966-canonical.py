#!/usr/bin/env python3
"""Benchmark P966 — conteúdo de função de utilizador em math recebe default (braços Sequence/Styled).

"antes" = binário release do estado pós-P957 (pré-P966), copiado para
temp/p966/typst-antes antes do rebuild. "depois" = release actual.
7 cenários canónicos, output para /dev/null. Estado da árvore: working tree
com as alterações de P966 (math/layout/mod.rs) sobre `552fcbdeb`
(ver relatório P966).
"""
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p966-canonical"
CORPUS_DIR = ROOT / "tools" / "perf" / "corpus" / "p922923-canonical"
ANTES_BIN = ROOT / "temp" / "p966" / "typst-antes"
...
def run(cmd, **kwargs):
    print(f"[p966] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        ...
        ratio = means["p966"] / means["antes"]
        rows.append((name, means["antes"], means["p966"], ratio))
        print(f"{name}: antes={means['antes']:.2f}ms depois={means['p966']:.2f}ms ratio={ratio:.3f}")
