#!/usr/bin/env python3
"""Benchmark P932-lazy — coverage calculada no arranque (estilo vanilla).
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p932-lazy"
...
def run(cmd, **kwargs):
    print(f"[p932-lazy] {' '.join(str(c) for c in cmd)}")
...
def benchmark_suite(suite_name: str, docs, corpus_dir: Path, warmup: int, min_runs: int):
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p932-lazy] SKIP {suite_name}/{name}: ficheiro não encontrado: {doc}")
            continue
        out = RESULTS_DIR / suite_name / f"{name}.json"
        print(f"[p932-lazy] {suite_name}/{name}", flush=True)
        hf = run_hyperfine(name, doc, out, warmup, min_runs)
        p927_ms = hf.get("p927", 0.0)
        proto_ms = hf.get("p932-lazy", 0.0)
        ratio = proto_ms / p927_ms if p927_ms > 0 else float("inf")
        rows.append({
            "name": name,
            "p927_ms": p927_ms,
            "p932_lazy_ms": proto_ms,
            "ratio": ratio,
        })
        print(f"[p932-lazy] {suite_name}/{name}: p927={p927_ms:.2f}ms p932-lazy={proto_ms:.2f}ms ratio={ratio:.2f}x")
    return rows
...
def main():
    ...
    attestation = {
        ...
        "binaries": {
            ...
            "p932_lazy": {"path": str(PROTO_BIN), "sha256": sha256_file(PROTO_BIN)},
        },
        "canonical": canonical,
        "utf8": utf8,
    }
    ...
    print(f"\n[p932-lazy] Attestation escrita em {attestation_path}")
