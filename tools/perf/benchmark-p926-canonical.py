#!/usr/bin/env python3
"""Benchmark P926 — impacto no caso comum da pré-computação de coverage.

Compara o binário cristalino original (commit `da18ea9f3`, sem protótipo)
contra o binário do protótipo (working tree com coverage pré-computada).
...
RESULTS_DIR = ROOT / "tools" / "perf" / "results" / "p926-canonical"
...
def run(cmd, **kwargs):
    print(f"[p926] {' '.join(str(c) for c in cmd)}")
...
def main():
    ...
    for name, doc in docs:
        if not doc.exists():
            print(f"[p926] SKIP {name}: ficheiro não encontrado: {doc}")
            continue
        print(f"[p926] {name}", flush=True)
        ...
        print(f"[p926] {name}: original={original_ms:.2f}ms proto={proto_ms:.2f}ms ratio={ratio:.2f}x")

    attestation = {
        "passo": "P926-canonical",
        "warmup": WARMUP,
        "min_runs": MIN_RUNS,
        "binaries": {
    ...
    print(f"\n[p926] Attestation escrita em {attestation_path}")
