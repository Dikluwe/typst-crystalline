...
pub(super) fn eval_module_include<'r>(
    ...
    let path_val = eval_expr(include.source(), scopes, ctx, route, styles, show_rules, active_guards)?;
    ...
    let result = eval_markup(source.root(), scopes, ctx, child_route.track(), styles, show_rules, active_guards);
