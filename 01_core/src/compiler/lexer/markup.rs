//! Crystalline Lineage
//! @prompt 00_nucleo/prompts/compiler/lexer/markup.md
//! @prompt-hash 1a28a2fc
//! @layer L1
//! @updated 2026-04-23
//!
    ...
    fn text(&mut self) -> SyntaxKind {
        ...
        loop {
            ...
            match s.eat() {
                ...
                // Spaces end the current Text token; the main lexer loop will
                // emit a separate Space / Parbreak token via whitespace().
                Some('/') if !s.at(['/', '*']) => {}
                Some('-') if !s.at(['-', '?']) => {}
                Some('.') if !s.at("..") => {}
                Some('h')
