...
mod slicing;
use crate::compiler::layout::helpers::{item_pos, resolve_pt, translate_frame_item};
...
pub struct Layouter<'a, M: FontMetrics, S: ImageSizer = NullImageSizer> {
    ...
    /// `00_nucleo/prompts/compiler/layout.md` §P908, `resolve_path_slice`.
...
pub(super) struct DeferredFloat {
    ...
    /// novo. Ver `00_nucleo/prompts/compiler/layout.md` §P908.
...
>(
    ...
    let bib_render_cache = bib_style.and_then(|style| {
        let result = if let Some(resolved) = style.resolved_style {
            crate::compiler::layout::bib_csl::build_cache_with_style(
            ...
            crate::compiler::layout::bib_csl::build_cache(
                introspector.bib_store.entries(),
                Some(name),
                style.locale.as_deref(),
                Some(citation_order),
