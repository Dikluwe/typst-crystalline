//! Crystalline Lineage
//! @prompt 00_nucleo/prompts/compiler/eval/bibliography.md
//! @prompt-hash ffc57a00
//! @layer L1
//! @updated 2026-06-23
//!
...
//! via path local `.csl` XML. Reexporta helpers de `compiler/layout/bib_csl.rs`.
...
pub fn parse_bibliography(content: &str, path: &str) -> SourceResult<Vec<BibEntry>> {
    ...
    let library: hayagriva::Library = match ext.as_str() {
        "bib" => {
            ...
            return crate::compiler::eval::bibtex::parse_bibtex(content).map_err(|e| {
                vec![SourceDiagnostic::error(
                    Span::detached(),
...
pub fn resolve_style(
    ...
    if let Some(style) = crate::compiler::layout::bib_csl::resolve_style_name(style_str) {
        return Ok(style);
    }
...
fn load_csl_style_from_path(
    ...
    crate::compiler::layout::bib_csl::parse_csl_style(content).map_err(|e| {
        vec![SourceDiagnostic::error(
            Span::detached(),
