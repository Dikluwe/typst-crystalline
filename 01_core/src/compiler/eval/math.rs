                ...
                "cases" => {
                    ...
                    for arg in call.args().items() {
                        match arg {
                            ...
                            _ => {} // neutro: N16[β] — argumentos posicionais em vec() não são delim e são avaliados como células math
                        }
                    }
...
fn parse_delim_val(val: &Value) -> Option<(char, char)> {
    match val {
        ...
        _ => None, // neutro: N16[β] — projecção de delim: valores não-string/array retornam None (paridade vanilla math/matrix.rs)
    }
}
