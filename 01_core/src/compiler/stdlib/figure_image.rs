...
fn infer_kind_from_body(body: &Content) -> Option<String> {
    match body {
        ...
        _ => None, // neutro: N16[β] — Content sem tipo inferível retorna None
    }
}
...
pub fn native_figure(
    ...
    let kind: Option<String> = args
        ...
        .and_then(|v| match v {
            ...
            _ => None, // tipo inválido — cai em fallback // neutro: N16[β] — tipo inválido de kind retorna None para fallback
        })
