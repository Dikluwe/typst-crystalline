...
mod tests {
    ...
    fn keywords_are_keywords() {
        ...
        assert!(SyntaxKind::For.is_keyword());
        assert!(SyntaxKind::Return.is_keyword());
        assert!(SyntaxKind::Import.is_keyword());
    }

    #[test]
    fn non_keywords_are_not_keywords() {
        assert!(!SyntaxKind::Text.is_keyword());
        assert!(!SyntaxKind::Ident.is_keyword());
        assert!(!SyntaxKind::Int.is_keyword());
    }
    ...
    fn trivia_detection() {
        ...
        assert!(SyntaxKind::Parbreak.is_trivia());
    ...
    fn error_detection() {
        ...
        assert!(!SyntaxKind::Text.is_error());
    }

    #[test]
    fn grouping_detection() {
        assert!(SyntaxKind::LeftBracket.is_grouping());
        assert!(SyntaxKind::RightParen.is_grouping());
        assert!(!SyntaxKind::Comma.is_grouping());
    }

    #[test]
    fn names() {
        assert_eq!(SyntaxKind::Text.name(), "text");
        assert_eq!(SyntaxKind::Error.name(), "syntax error");
        assert_eq!(SyntaxKind::Let.name(), "keyword `let`");
    }
}
