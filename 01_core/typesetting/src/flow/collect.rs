...
use primitives::foundations::{Packed, Resolve, Smart, StyleChain};
...
use primitives::model::ParElem;
use primitives::routines::{Pair, Routines};
use primitives::text::TextElem;
use ::shared::{Protected, SliceExt};
...
impl<T> CachedCell<T> {
    ...
    where
        ...
        F: FnOnce(I) -> T,
    {
        let input_hash = shared::hash128(&input);
