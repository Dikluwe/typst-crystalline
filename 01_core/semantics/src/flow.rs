use primitives::diag::{At, SourceDiagnostic, SourceResult, bail, error};
use primitives::foundations::{IntoValue, Value, ops};
use lexicon::ast::{self, AstNode};
use lexicon::{Span, SyntaxKind, SyntaxNode};
...
impl Eval for ast::WhileLoop<'_> {
    ...
    #[metaprogramming::time(name = "while loop", span = self.span())]
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
...
impl Eval for ast::ForLoop<'_> {
    ...
    #[metaprogramming::time(name = "for loop", span = self.span())]
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
        let flow = vm.flow.take();
        let mut output = Value::None;
