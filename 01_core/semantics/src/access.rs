use ecow::eco_format;
use primitives::diag::{At, Hint, SourceResult, Trace, Tracepoint, bail};
use primitives::foundations::{Dict, Value};
...
pub(crate) fn access_dict<'a>(
    ...
    match access.target().access(vm)? {
        ...
        value => {
            ...
            } else if primitives::foundations::fields_on(ty).is_empty() {
