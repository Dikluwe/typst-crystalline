...
use lexicon::{Span, Spanned, SyntaxMode};
use shared::{ManuallyHash, NonZeroExt, PicoStr};
...
impl Bibliography {
    ...
    fn decode(data: &[Loaded]) -> SourceResult<Bibliography> {
        ...
        Ok(Bibliography(Arc::new(ManuallyHash::new(map, shared::hash128(data)))))
    }
    ...
    pub fn from_archived(archived: ArchivedStyle) -> CslStyle {
        match archived.get() {
            citationberg::Style::Independent(style) => Self(Arc::new(ManuallyHash::new(
                style,
                shared::hash128(&(TypeId::of::<ArchivedStyle>(), archived)),
            ))),
        ...
        citationberg::IndependentStyle::from_xml(text)
            .map(|style| {
                Self(Arc::new(ManuallyHash::new(
                    style,
                    shared::hash128(&(TypeId::of::<Bytes>(), bytes)),
                )))
            })
            .map_err(|err| {
