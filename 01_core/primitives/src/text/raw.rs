...
use lexicon::{LinkedNode, Span, Spanned, split_newlines};
use shared::ManuallyHash;
    ...
    fn highlight(&self, routines: &Routines, styles: StyleChain) -> Vec<Packed<RawLine>> {
        ...
        if matches!(lang.as_deref(), Some("typ" | "typst" | "typc" | "typm")) {
            ...
            let root = match lang.as_deref() {
                Some("typc") => lexicon::parse_code(&text),
                Some("typm") => lexicon::parse_math(&text),
                _ => lexicon::parse(&text),
            };
...
impl RawTheme {
    ...
    fn decode(bytes: &Bytes) -> LoadResult<RawTheme> {
        ...
        Ok(RawTheme(Arc::new(ManuallyHash::new(theme, shared::hash128(bytes)))))
    }
...
impl<'a> ThemedHighlighter<'a> {
    ...
    fn highlight_inner(&mut self) {
        ...
        for child in self.node.children() {
            ...
            if let Some(tag) = lexicon::highlight(&child) {
...
fn align_tabs(text: &str, tab_size: usize) -> EcoString {
    ...
    for grapheme in text.graphemes(true) {
        ...
        if c == Ok('\t') {
            ...
            column += required;
        } else if c.is_ok_and(lexicon::is_newline) || grapheme == "\r\n" {
