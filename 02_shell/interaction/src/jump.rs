use ecow::EcoVec;
use fusion::introspection::{DocumentPosition, HtmlPosition};
use fusion::layout::{Frame, FrameItem, PagedDocument, Point, Position, Size};
use fusion::model::{Destination, Url};
...
mod jump_from_document_sealed {
    ...
    impl JumpFromDocument for HtmlDocument {
        ...
        fn resolve_position(
            ...
            let is_text_node =
                ast_node.is_some_and(|x| x.is::<fusion::syntax::ast::Text>());
...
mod tests {
    ...
    fn test_click(world: impl WorldLike, click: Point, expected: Option<Jump>) {
        ...
        let doc: PagedDocument = fusion::compile(world).output.unwrap();
    ...
    fn test_click_html(
        ...
        let world = world.acquire();
        let world = world.borrow();
        let doc: HtmlDocument = fusion::compile(world).output.unwrap();
