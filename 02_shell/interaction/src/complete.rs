...
fn path_completion(func: &Func, param: &ParamInfo) -> Option<&'static [&'static str]> {
    Some(match (func.name(), param.name) {
        ...
        (Some("embed"), "path") => &[],
        (Some("attach"), "path") if *func == fusion::pdf::AttachElem::ELEM => &[],
        (None, "path") => &[],
        _ => return None,
    })
}
...
impl<'a> CompletionContext<'a> {
    ...
    fn cast_completions(&mut self, cast: &'a CastInfo) {
        ...
        if !self.seen_casts.insert(fusion::utils::hash128(cast)) {
            return;
        }
