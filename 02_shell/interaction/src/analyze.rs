...
use fusion::foundations::{Label, Styles, Value};
use fusion::layout::PagedDocument;
use fusion::model::{BibliographyElem, FigureElem};
use fusion::syntax::{LinkedNode, SyntaxKind, ast};
...
pub fn analyze_expr(
    ...
    let val = match expr {
        ...
        _ => {
            ...
            return fusion::trace::<PagedDocument>(world.upcast(), node.span());
        }
    };
...
pub fn analyze_import(world: &dyn IdeWorld, source: &LinkedNode) -> Option<Value> {
    ...
    crate::utils::with_engine(world, |engine| {
        semantics::import(engine, &path, source_span).ok().map(Value::Module)
    })
}
