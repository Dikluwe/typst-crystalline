...
use primitives::diag::{SourceDiagnostic, error};
use primitives::foundations::{Content, Packed, Smart};
use primitives::layout::Length;
use primitives::text::{
    HighlightElem, OverlineElem, ScriptKind, StrikeElem, SubElem, SuperElem, TextItem,
    TextSize, UnderlineElem,
};
use primitives::visualize::Stroke;
...
impl TextParams {
    fn new(text: &TextItem) -> TextParams {
        TextParams(shared::hash128(&(&text.font, text.size)))
    }
}
