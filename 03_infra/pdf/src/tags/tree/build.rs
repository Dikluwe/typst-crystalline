...
//!
//! [`introspection::Tag`]: primitives::introspection::Tag
//! [`FrameItem::parent`]: primitives::layout::FrameItem
...
use primitives::foundations::{Content, ContextElem};
...
use primitives::math::EquationElem;
...
use primitives::pdf::{ArtifactElem, PdfMarkerTag, PdfMarkerTagKind};
use primitives::text::{
    HighlightElem, OverlineElem, RawElem, RawLine, StrikeElem, SubElem, SuperElem,
    UnderlineElem,
};
use primitives::visualize::ImageElem;
use lexicon::Span;
...
fn visit_frame(tree: &mut TreeBuilder, frame: &Frame) -> SourceResult<()> {
    for (_, item) in frame.items() {
        match item {
            ...
            FrameItem::Tag(primitives::introspection::Tag::Start(elem, flags)) => {
                if flags.tagged {
                    visit_start_tag(tree, elem);
                }
            }
            FrameItem::Tag(primitives::introspection::Tag::End(loc, _, flags)) => {
                if flags.tagged {
...
/// [primitives::layout::GroupItem::parent] has been set. All elements that
