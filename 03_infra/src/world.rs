...
impl World for SystemWorld {
    ...
    /// **P935** — cobertura Unicode disponível directamente no `FontBook`.
    ///
    /// `font_info_from_bytes` preenche `coverage` eager durante a descoberta de
    /// fontes (padrão vanilla), pelo que `candidates_for_char` delega simplesmente
    /// a `FontBook::candidates_for_char`.
