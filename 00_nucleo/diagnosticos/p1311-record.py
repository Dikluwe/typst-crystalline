"""Bounded P1311 commands with immutable, full provenance receipts."""
import datetime
import hashlib
import json
import os
...
TARGET = '/dev/shm/p1311-target.JE8Cyy'
BASE_BINARY = '/dev/shm/p1310-target.VeBP6Q/release/typst'
...
def init():
    s = state()
    assert not s['staged']
    previous = json.loads((D/'p1310-verification.json').read_text())
    assert s['head'] == previous['baseline']['head']
    for path, expected in [('01_core/src/compiler/stdlib/loading.rs', previous['candidate']['source_sha256']), ('00_nucleo/prompts/compiler/stdlib/loading.md', previous['candidate']['L0_sha256'])]:
        assert sha(ROOT/path) == expected, path
    paths = json.loads((D/'p1310-baseline.json').read_text())['files']
    files = {p: sha(ROOT/p) for p in paths}
    prior = {str(p.relative_to(ROOT)): sha(p) for prefix in ('p1309-*','p1310-*') for p in D.glob(prefix) if p.is_file()}
...
def command(name, argv):
    ...
    data = {'before': before, 'after': state(), 'argv': argv, 'cwd': str(ROOT),
            ...
            'baseline_sha256': sha(D/'p1311-baseline.json'),
