...
def preservation_review():
    r = preflight()
    baseline = read('p1322-baseline.json')
    for name, digest_expected in baseline['untracked_at_start'].items():
        r.require(name.startswith('00_nucleo/diagnosticos/'), 'UNTRACKED_SCOPE', name)
        if name.startswith('00_nucleo/diagnosticos/'):
    ...
    r.require(digest(ROOT/'00_nucleo/materialization/typst-passo-1322.md')==baseline['step_sha256'], 'STEP_CHANGED', 'P1322')
    ...
    return r, {'historical_untracked_preserved':len(baseline['untracked_at_start']),
...
def query_rejection_controls():
    r = Review()
    data = read('p1322-transversal-r2.json')
...
def transversal_review(suffix=''):
    r = preflight()
    data = read('p1322-transversal'+suffix+'.json')
    freeze_name = 'p1322-transversal'+suffix+'-freeze.json'
    frozen = read(freeze_name)
    corpus = read('p1322-transversal-cases.json')
