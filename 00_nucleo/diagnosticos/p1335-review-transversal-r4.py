...
def cli_absent(row,corpus,frozen,outroot):
    if row.get('id') not in ('html-legacy','preserve-pdf') or row.get('phase') not in ('normal','repeat','reverse'):return False
    case=next(x for x in corpus['warning_cases'] if x['id']==row['id'])
    o=row.get('observations',{}).get('vanilla',{});binary=frozen['binaries']['vanilla']
    source=Path(corpus['html'])/'hello.typ'
    output=Path(outroot)/'warning'/f"{row['phase']}-vanilla-{row['id']}.{case['format']}"
    args=[x.replace(str(D/'p1323-fixtures'),corpus['html']).replace('{output}',str(output)) for x in case['argv']]
    if any((row.get('case')!=case,o.get('command')!=[binary['path'],*args],o.get('binary_sha256')!=binary['sha256'],
            o.get('cwd')!=str(c.ROOT),o.get('stdin_base64')!=case['stdin_base64'],o.get('stdout')!='',o.get('exit_code')!=2,o.get('artifact') is not None)):return False
    if not source.is_file() or c.digest(source)!=frozen['inputs'].get(str(source)):return False
    if row['id']=='html-legacy':
...
def run(focal):
    ...
    for row in data['warnings']:
        ...
        for side,o in row['observations'].items():
            ...
            if case['warning_expected'] and not (side=='vanilla' and cli_absent(row,corpus,frozen,data['output_root'])):r.require(err.count(fixed.splitlines()[0])==1,'WARNING_DUPLICATED',[key,side])
            if not o['complete'] and not (side=='vanilla' and cli_absent(row,corpus,frozen,data['output_root'])):r.unknown('WARNING_EXECUTION_UNKNOWN',[key,side])
    r.require(set(wrows)=={(id_,phase) for id_ in wexpected for phase in phases},'WARNING_COVERAGE',len(wrows))
    cli_controls=[];overlay=c.read('p1335-cli-capability-overlay.json')
    for path,pin in overlay['inputs'].items():r.require(c.digest(path)==pin,'CLI_OVERLAY_PIN',path)
    recognized=[]
    for row in data['warnings']:
        if cli_absent(row,corpus,frozen,data['output_root']):
            recognized.append((row['id'],row['phase'],'vanilla'))
            if row['phase']=='normal':
                for mode in ('genuine','missing','truncated','generic','crash','stdout','command','binary','artifact'):
                    altered=copy.deepcopy(row);o=altered['observations']['vanilla']
                    if mode=='missing':o.pop('stderr')
                    if mode=='truncated':o['stderr']=o['stderr'].splitlines()[0]
                    if mode=='generic':o['stderr']='error: failed\\n'
                    if mode=='crash':o['exit_code']=-9
                    if mode=='stdout':o['stdout']='output'
                    if mode=='command':o['command'][1]='eval'
                    if mode=='binary':o['binary_sha256']='0'*64
                    if mode=='artifact':o['artifact']={'path':'fabricated'}
                    actual='PUBLIC_CLI_ABSENT' if cli_absent(altered,corpus,frozen,data['output_root']) else 'Unknown'
                    expected='PUBLIC_CLI_ABSENT' if mode=='genuine' else 'Unknown'
                    r.require(actual==expected,'CLI_ABSENCE_CONTROL',[row['id'],mode])
                    cli_controls.append(dict(id=row['id'],mode=mode,actual=actual,expected=expected))
    if not focal:
        r.require(set(recognized)=={(x['id'],x['phase'],x['side']) for x in overlay['rows']},'CLI_OVERLAY_COVERAGE',recognized)
        r.require(all(x['successor_state']=='PUBLIC_CLI_ABSENT' and x['original_complete']==False for x in overlay['rows']),'CLI_OVERLAY_CLASS',None)
        for key,row in wrows.items():
            if key[1]=='normal':continue
            normal=wrows[key[0],'normal']
            for side,o in row['observations'].items():
                for channel in ('stdout','stderr','exit_code'):
                    r.require(o[channel]==normal['observations'][side][channel],'WARNING_CHANNEL_STABILITY',[key,side,channel])
    controls=[]
    ...
    return dict(at=c.utc(),role='D',manifest_sha256=c.digest(D/'p1335-manifest.json'),checker_sha256=c.digest(__file__),inputs=c.INPUTS,counts=summaries,projected_match_raw_differences=projected_raw_diff,query_controls=controls,cli_capability_controls=cli_controls,cli_capability_observations=recognized,
