# Histórico — antigo Prompt L0 agregado de eval
...
- `Expr::CodeBlock` → `scopes.enter()` antes do loop, evalua exprs
  ...
  inválidas → erro; ver `compiler/eval/operators/join.md`)
...
(`eval_func_call` em `compiler/eval/closures.rs`):
...
- **#56 — âncora de `eval`**: quando o callee avalia para o nativo `eval`
  ...
  (débito P772s). Detalhes e ressalvas em `prompts/compiler/stdlib/eval.md` §3.
...
Criar `01_core/src/compiler/eval/flow.rs` em L1 com:
...
- `00_nucleo/adr/typst-adr-0101-ieee754-restricao-stdlib.md` — política
  formalizada.
- `00_nucleo/prompts/compiler/stdlib.md` §"Política IEEE 754" — sítio
...
- A tabela de dados e os grupos `join`/`bowtie` vivem em `compiler/stdlib/sym.rs` (L0 `sym.md` §7); scope-out das 13 depreciações ao nível de variante registado lá.
...
Regras para `eval_set_rule` com target `"text"` (`01_core/src/compiler/eval/rules.rs`):
...
- `Value::Length` é aceite e propagado pelo canal custom (`text.top-edge`/`text.bottom-edge`) até `TextStyle` como `TextEdge::Length` — ver `compiler/layout.md` §P762/P837 para o tipo `TextEdge` e a semântica de resolução (`length.resolve_pt(size)` a partir da baseline; bottom negativo = abaixo da baseline).
...
- O layout (`compiler/layout/text.rs`) lê `"text.style"` e traduz `"italic"`/`"oblique"` para `italic = true`, coexistente com o campo tipado `italic`.
...
Semântica a implementar em `eval_module_import` (`01_core/src/compiler/eval/modules.rs`),
...
2. Dentro: `extract_measure_body` (validação de argumentos, partilhada
   ...
   (`layout/mod.rs` §P712, `compiler/layout.md` §"`measure_content_real`") —
...
`Expr::ContentBlock` em `01_core/src/compiler/eval/mod.rs` construíam
...
`MathFieldAccess`). `compiler/lexer/math.rs` (cristalino) já replica isto
...
`Scopes::get_local` (novo, `compiler/scopes.rs`, ver `scopes.md` §P780) —
...
`value_to_display_content` (novo, `compiler/eval/mod.rs`) — extraído do
...
`unknown_variable_math(span, name, in_global)` (novo, `compiler/eval/
...
(`compiler/math/symbols.rs`), só `prod` (nome **não-canónico** — `codex`
...
`compiler/parse/math.rs:63-64` (`SyntaxKind::MathIdent | SyntaxKind::
...
`compiler/parse/markup.rs`, ver parse.md §P786a) **não abortam** — seguem para o
...
**Decisão:** em `eval_func_call` (`compiler/eval/closures.rs`), **depois** de
...
(`compiler/eval/bindings.rs`), mirror do ramo de erro de `eval_field_callee`
...
(`compiler/eval/bindings.rs`), despachados numa intercepção de `eval_func_call`
(`compiler/eval/closures.rs`) **antes** do fallback P815 e depois de todos os
...
**Decisão:** em `eval_math_expr` (`compiler/eval/math.rs`, braço `FuncCall`
...
Secção perene que consolida o conteúdo normativo de P792 no domínio de `compiler/eval`.
(A componente stdlib — `native_layout` em `compiler/stdlib/layout.rs` — é governada pelo
L0 da stdlib; as variantes `Content::Layout` e o arm no Layouter pertencem aos L0s de
`entities/content` e `compiler/layout`.)

### Registo de `layout` no scope global

`01_core/src/compiler/eval/mod.rs` importa `native_layout` e regista
...
## §Field Access em tipos primitivos, namespaces e módulos (absorvido de `compiler/eval/field-access.md`)

Consolidação perene do `match target` de `eval_field_access` /
`eval_value_field_access` em `01_core/src/compiler/eval/bindings.rs` (e de
...
## §Campos nativos e diagnósticos de campo (absorvido de `compiler/eval/fields.md`)
...
NÃO lexam como `SyntaxKind::Bool` — só `compiler/lexer/code.rs::keyword`
