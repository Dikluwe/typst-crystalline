...
def main():
 ...
 authorities=[
  ...
  pin('p1339-test-instrumentation-configuration.json','7979f51201809ae69a272a1b61b859d8801f1e6f543b244e4d1dbb2ad2bac6b9')]
 ...
 adapter_data=json.loads(Path(receipt['path']).read_text())
 for name,expected in adapter_data['runner_input_pins'].items():
  p=pin(name,expected);artifacts[p['path']]=p
 for item in adapter_data['component_and_ancestry_pins']:
  p=pin(item['path'],item['sha256']);artifacts[p['path']]=p
 ...
 data={'schema':'p1339-aggregate-batch-manifest-v1','batch':1,'author':'/root/p1319_tests','regime':'executado sem atestacao de isolamento',
  ...
  'predicate_classification_aggregation':{'consumer':'independent verifier; author comparison is evidence only','exit_zero':'Predicate process exit0 only means all lines emitted, never PASS. Parse every JSON classification row and require exact384-cell bijection with raw input.','ordinary_rows':'Every due mandatory ordinary row must classification Preserved with no differences. Any Violated, Unknown/mandatory_unknown, malformed/missing/extra row or checker exception rejects focal regardless of exit code.','opaque_rows':'Only12 declared vanilla workload cells may be Unknown, each reason declared_observation_budget_opacity and control_satisfied true, positive_credit0.12 baseline workload cells must known-error Preserved with control_satisfied true and positive_credit0.','gain_vector':'Record all384 classifications and reason codes, corrected/regressed cases against ancestral policy, actual cost and12future scenario definitions separately; no future pass count.'},
  'recorder_successor_reason':'Same batch1: integrate final adapter-r2, explicit predicate-classification aggregation requested by verifier, and prospective instrumentation configuration. Original recorder never ran or emitted a manifest.',
