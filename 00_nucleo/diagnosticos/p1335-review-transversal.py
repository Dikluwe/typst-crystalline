...
def run(focal):
    r=c.preflight();data=c.read('p1335-transversal-focal.json' if focal else 'p1335-transversal.json')
    frozen=c.read('p1335-transversal-freeze.json');corpus=c.read('p1335-transversal-cases.json')
    r.require(data['freeze_sha256']==c.digest(D/'p1335-transversal-freeze.json'),'TRANSVERSAL_FREEZE',focal)
    ...
    for group in data['matrix']:
        ...
        for row in group['results']:
            ...
            check_row(row,profile,phase,Path(corpus['temp']),Path(corpus['temp'])/phase/profile/row['id'])
    ...
    if not focal:
        ...
        for row in data['location']:
            check_row(row,row['profile'],row['phase'],Path(corpus['internal']),Path(corpus['temp'])/'location'/row['phase']/row['profile']/row['id'])
    ...
    for row in data['warnings']:
        ...
        for side,o in row['observations'].items():
            output=Path(corpus['temp'])/'warning'/f"{row['phase']}-{side}-{row['id']}.{case['format']}"
            ...
            err=o['stderr'];valid=(err.startswith(fixed) and (not case['error_expected'] or err[len(fixed):].startswith('error:'))) if case['warning_expected'] else 'warning: HTML export is under active development' not in err
            r.require(o['warning_contract']==valid,'WARNING_CONTRACT_CLASS',[key,side])
            r.require(valid,'WARNING_CONTRACT_NOT_PRESERVED',[key,side])
            if case['warning_expected']:r.require(err.count('warning: HTML export is under active development')==1,'WARNING_DUPLICATED',[key,side])
