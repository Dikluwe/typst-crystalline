...
def main():
    ...
    binaries = {"baseline": ("/dev/shm/p1312-target.B8uLa9/release/typst", "446c3ccfa5ef7f5eaf6031543f4d28566dac6eb53dca842ec7eb16acb084fbf0"), "vanilla": ("/usr/local/bin/typst", "7b4f40c56d6fa95082ebcfd893e275d418ebcaed1b97b62785f78284c63ff7b8")}
    if args.candidate:
    ...
    result = {"schema": "p1313-ab-runs-v1", "start": state(), "inputs": {"cases_sha256": sha(cases_path), "runner_sha256": sha(__file__), "l0_sha256": sha(L0), "fixtures": {str(p): sha(p) for p in FIXTURES}}, "binaries": {k: {"path": p, "sha256": s} for k, (p, s) in binaries.items()}, "runs": []}
    env = dict(os.environ)
        ...
        for profile, flags in PROFILES.items():
            for case in list(reversed(cases)) if order == "reverse" else cases:
                for product, (binary, digest) in binaries.items():
                    expression = case.get("surrogate", case["expr"]) if product == "baseline" and case["kind"] == "legacy-bytes" else case["expr"]
