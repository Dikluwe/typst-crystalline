"""Independent P1336 A/B runner. Frozen before C; no candidate source reads."""
import argparse
import base64
...
MANIFEST_SHA = "54864bfd67d50197560011d5aae055a0303fcc9cadc4230221dfd4f7a6daea88"
...
def main():
    ...
    assert sha(D / "p1336-manifest.json") == MANIFEST_SHA
    m, cases = read("p1336-manifest.json"), read("p1336-tests-cases.json")
    started = state()
    hashes = {name: sha(D/name) for name in ["p1336-tests-cases.json", "p1336-tests-runner.py"]}
    if args.mode == "baseline":
        products = {"vanilla": m["vanilla"], "baseline": m["baseline_binary"]}
    else:
        freeze = read("p1336-tests-freeze.json")
        assert hashes == freeze["protected"]
        assert sha(D/"p1336-tests-baseline.json") == freeze["baseline_runs_sha256"]
        assert sha(D/"p1336-tests-expectations.json") == freeze["expectations_sha256"]
    ...
    if args.finalize_baseline:
        ...
        measured = read("p1336-tests-baseline.json")
    ...
    if not args.finalize_baseline:
        save("p1336-tests-" + args.mode + ".json", result)
    ...
    if args.mode == "baseline":
        ...
        save("p1336-tests-expectations.json", expectations)
        save("p1336-tests-freeze.json", {"at":utc(), "manifest_sha256":MANIFEST_SHA,"protected":hashes,
             "baseline_runs_sha256":sha(D/"p1336-tests-baseline.json"), "expectations_sha256":sha(D/"p1336-tests-expectations.json"),
             ...
             "rust_tests":"01_core/src/compiler/eval/bindings/field_access.rs::p1336_tests; frozen before implementation",
        ...
        expectations = {(e["id"],e["profile"]):e for e in read("p1336-tests-expectations.json")}
        ...
        save("p1336-tests-comparison.json", {"at":utc(),"manifest_sha256":MANIFEST_SHA,
             "candidate_runs_sha256":sha(D/"p1336-tests-candidate.json"),"freeze_sha256":sha(D/"p1336-tests-freeze.json"),"rows":comparisons})
