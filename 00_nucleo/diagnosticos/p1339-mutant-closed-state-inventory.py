...
OUTPUT = D/'p1339-mutant-closed-state-inventory.json'
...
for path in sources:
    ...
    for i,line in enumerate(lines):
        ...
        declarations.append({'name':name,'kind':kind,'path':str(path.relative_to(ROOT)),
                             ...
                             'type_tokens':sorted(set(re.findall(r'\b[A-Z][A-Za-z0-9_]*\b',joined)))})
by_name={}
...
while pending:
    ...
    for item in items:
        ...
        selected[key]=item
        tokens=set(item['type_tokens'])-{item['name']}-{v['name'] for v in item['direct_variants']}
        pending.extend(tokens)
head=subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,capture_output=True,text=True,check=True).stdout.strip()
record={
    'schema':'p1339-baseline-declaration-inventory-v1','at':datetime.datetime.now(datetime.timezone.utc).isoformat(),
