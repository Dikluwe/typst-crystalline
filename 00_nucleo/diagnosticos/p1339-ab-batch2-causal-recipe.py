...
PLAN=D/'p1339-ab-batch2-cli-plan.json'
...
def derive(raw):
    ...
    for profile in ['default','html','a11y','html+a11y']:
        ...
        for order in ['normal','repeat','reverse']:
            ...
            spanless={'exit':1,'stdout':'','stderr':'error: only element functions can be used as selectors\n\n'}
            ...
            results.append({'profile':profile,'order':order,'raw_row_index':index,'source_case':recipe['calibration_case_id'],'target_case':'where-l0-empty-vs-bare','candidate_expected':expected,'byte_mapping':mapping})
...
if __name__=='__main__':
    ...
    assert raw_path.parent==D and raw_path.name=='p1339-ab-batch2-focal-runs.json'
    assert out.parent==D and out.name=='p1339-ab-batch2-causal-derived-expectations.json' and not out.exists()
