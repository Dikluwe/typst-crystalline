#!/usr/bin/env python3
"""P1335 D: independent audit of receipts; never imports the judged runner.

Rules frozen in p1335-review-plan.md. Data adapters are explicit, not oracles.
Only audit artifacts bearing p1335-review- may be written by this program.
"""
import argparse
import base64
import collections
...
def preflight():
    r = Review()
    baseline, manifest, build = (read('p1335-' + x + '.json')
                                 for x in ('baseline', 'manifest', 'build'))
    r.require(manifest['baseline_sha256'] == digest(DIAG/'p1335-baseline.json'),
              'BASELINE_PIN', 'p1335-manifest.json')
    r.require(build['manifest_sha256'] == digest(DIAG/'p1335-manifest.json'),
              'MANIFEST_PIN', 'p1335-build.json')
...
def preservation_review():
    r = preflight()
    baseline = read('p1335-baseline.json')
    for name, digest_expected in baseline['historical_preserved'].items():
        r.require(pathlib.Path(name).is_relative_to(DIAG), 'UNTRACKED_SCOPE', name)
        if pathlib.Path(name).is_relative_to(DIAG):
            r.require((ROOT/name).is_file() and digest(ROOT/name)==digest_expected, 'HISTORICAL_EVIDENCE_CHANGED', name)
    for name, digest_expected in baseline['inputs'].items():
        r.require(pathlib.Path(name).is_file() and digest(name)==digest_expected, 'HISTORICAL_PIN_CHANGED', name)
    r.require(digest(ROOT/'00_nucleo/materialization/typst-passo-1322.md')==baseline['step_sha256'], 'STEP_CHANGED', 'P1335')
    for field, argv in [('head',['git','rev-parse','HEAD']), ('branch',['git','branch','--show-current']),
                        ('diff',['git','diff','HEAD','--binary']), ('staged',['git','diff','--cached','--binary'])]:
        measured = subprocess.check_output(argv, cwd=ROOT, text=True)
        if field in ('head','branch'):
            measured = measured.strip()
        r.require(measured==baseline['state'][field], 'BASELINE_STATE_CHANGED', field)
    return r, {'historical_untracked_preserved':len(baseline['historical_preserved']),
               'product_files_preserved':len(baseline['state']['product_inventory']),
               'historical_input_pins_preserved':len(baseline['inputs']), 'step_pin_preserved':True}
...
def verify_catalog(catalog, reconciliation, history, inventories):
    ...
    symbol_paths = {p['path'] for p in probes if p['path'].startswith(('sym.', 'emoji.'))}
    historical_paths = {p['path'] for p in history['probes']}
    r.require(symbol_paths <= discovered | historical_paths, 'SYMBOL_ROUTE_FABRICATED', sorted(symbol_paths - discovered - historical_paths))
    return r


def runtime_review():
    r = preflight()
    catalog = read('p1335-probe-catalog.json')
    reconciliation = read('p1335-inventory-reconciliation.json')
    history = read('p1322-probe-catalog.json')
    inventories = []
    ...
    frozen = read('p1335-runtime-freeze.json')
    ...
    for phase in ('normal', 'repeat', 'reverse'):
        path = DIAG/f'p1335-matrix-{phase}.json'
        if not path.exists():
            r.unknown('MATRIX_NOT_AVAILABLE', phase)
            continue
        matrix = read(path.name)
        r.require(matrix['catalog_sha256'] == digest(DIAG/'p1335-probe-catalog.json'),
                  'MATRIX_CATALOG_PIN', phase)
        r.require(matrix['freeze_sha256'] == digest(DIAG/'p1335-runtime-freeze.json'),
                  'MATRIX_FREEZE_PIN', phase)
        for moment in ('before', 'after'):
            r.require(matrix[moment]['product_inventory'] == read('p1335-baseline.json')['state']['product_inventory'],
...
def gate_review():
    r = preflight()
    summaries = {}
    baseline = read('p1335-baseline.json')['state']['product_inventory']
    for name in ('build', 'workspace-tests', 'fmt', 'lint', 'lineage', 'diff-check'):
        path = DIAG/f'p1335-{name}.json'
        if not path.exists():
            r.unknown('GATE_NOT_AVAILABLE', name)
            continue
        data = read(path)
        r.require(data.get('exit') == 0, 'GATE_FAILED', [name, data.get('exit')])
        r.require(data.get('manifest_sha256') == digest(DIAG/'p1335-manifest.json'),
                  'GATE_MANIFEST', name)
...
def supplement_review():
    r = preflight()
    frozen = read('p1335-sentinels-freeze.json')
    corpus = read('p1335-sentinels-cases.json')
    ...
    matrices, summaries = {}, {}
    for phase in ('normal', 'repeat', 'reverse'):
        path = DIAG/f'p1335-sentinels-{phase}.json'
        if not path.exists():
            r.unknown('SUPPLEMENT_ORDER_NOT_AVAILABLE', phase)
            continue
        matrix = read(path)
        r.require(matrix['freeze_sha256'] == digest(DIAG/'p1335-sentinels-freeze.json'),
...
QUERY_FEATURE_REJECTION = "error: unexpected argument '--features' found\n\n  tip: to pass '--features' as a value, use '-- --features'\n\nUsage: typst query --format <FORMAT> <INPUT> <SELECTOR>\n\nFor more information, try '--help'.\n"


def known_query_rejection(item):
    argv = item.get('command', [])
    return (item.get('exit_code') == 2 and item.get('stdout') == '' and
            item.get('stderr') == QUERY_FEATURE_REJECTION and len(argv)>1 and
            argv[1]=='query' and '--features' in argv)


def query_rejection_controls():
    r = Review()
    data = read('p1335-transversal-r2.json')
    genuine = next(row['crystalline'] for group in data['matrix']
                   if group['phase']=='normal' and group['profile']=='html'
                   for row in group['results'] if row['id']=='P1137-I-001')
    results = []
    for name in ('genuine_public_rejection', 'stderr_absent', 'stderr_truncated', 'other_exit2', 'crash', 'other_command'):
        item = copy.deepcopy(genuine)
        if name == 'stderr_absent':
            item.pop('stderr')
        elif name == 'stderr_truncated':
            item['stderr'] = item['stderr'].splitlines()[0]
        elif name == 'other_exit2':
            item['stderr'] = 'error: unclassified execution error\n'
        elif name == 'crash':
            item['exit_code'] = -9
        elif name == 'other_command':
            item['command'][1] = 'eval'
        observed = 'PUBLIC_CLI_ABSENT' if known_query_rejection(item) else 'Unknown'
        expected = 'PUBLIC_CLI_ABSENT' if name=='genuine_public_rejection' else 'Unknown'
        r.require(observed == expected, 'QUERY_POLICY_CONTROL', [name, observed, expected])
        results.append({'name':name,'expected':expected,'observed':observed})
    return r, {'focal_controls':results, 'policy_sha256':digest(DIAG/'p1322-review-query-exit2-policy.md')}


def transversal_review(suffix=''):
    r = preflight()
    data = read('p1335-transversal'+suffix+'.json')
    freeze_name = 'p1335-transversal'+suffix+'-freeze.json'
    frozen = read(freeze_name)
    corpus = read('p1335-transversal-cases.json')
    cases = {c['id']: c for c in corpus['matrix']['cases']}
    r.require(data['freeze_sha256'] == digest(DIAG/freeze_name), 'TRANSVERSAL_FREEZE', data['freeze_sha256'])
    for p, h in {**frozen['inputs'], **data['artifacts']}.items():
        r.require(pathlib.Path(p).is_file() and digest(p) == h, 'TRANSVERSAL_ARTIFACT_CHANGED', p)
    rows = {}
    summaries = {}
    for group in data['matrix']:
        phase, profile = group['phase'], group['profile']
        r.require({x['id'] for x in group['results']} == set(cases), 'TRANSVERSAL_CASE_COVERAGE', [phase, profile])
        counts = collections.Counter(row['estado'] for row in group['results'])
        r.require(dict(counts) == {k:v for k,v in group['counts'].items() if v}, 'TRANSVERSAL_COUNTS', [phase, profile])
        summaries[phase+'/'+profile] = dict(counts)
        for row in group['results']:
            key = (row['id'], profile, phase)
            r.require(key not in rows, 'TRANSVERSAL_DUPLICATE', key)
            rows[key] = row
            if row['estado'] == 'UNKNOWN':
                r.unknown('TRANSVERSAL_UNKNOWN', key)
            required = set(cases[row['id']].get('required_features', []))
            if not required <= set(PROFILES[profile]):
                r.require(row['estado'] == 'DISABLED_BY_PROFILE', 'TRANSVERSAL_FEATURE_GATE', key)
                continue
            for role, side in [('oracle', 'vanilla'), ('crystalline', 'crystalline')]:
                item = row.get(role, {})
                argv = item.get('command', [])
                r.require(bool(argv) and argv[0] == frozen['binaries'][side]['path'], 'TRANSVERSAL_BINARY', [key, side])
                features = set()
                for i, arg in enumerate(argv):
                    if arg == '--features' and i+1 < len(argv):
                        features.update(argv[i+1].split(','))
                r.require(features == set(PROFILES[profile]), 'TRANSVERSAL_PROFILE', [key, side, sorted(features)])
                if ((item.get('exit_code') not in (0, 1) and not known_query_rejection(item))
                        or any(k not in item for k in ('stdout', 'stderr'))):
                    r.unknown('TRANSVERSAL_EXECUTION_UNKNOWN', [key, side])
                if known_query_rejection(item):
                    r.require(row['estado']=='DIFFERENCE', 'CLI_REJECTION_PROMOTED', key)
    expected = {(id_, profile, phase) for id_ in cases for profile in PROFILES for phase in ('normal','repeat','reverse')}
    r.require(set(rows) == expected, 'TRANSVERSAL_ORDERS', sorted(expected-set(rows)))
    def stable_observed(value):
        if isinstance(value, dict):
            return {k: stable_observed(v) for k,v in value.items() if k not in ('diff_artifact',)}
        if isinstance(value, list):
            return [stable_observed(v) for v in value]
        return value
    for id_, profile, phase in sorted(expected):
        if phase == 'normal' or (id_, profile, phase) not in rows:
            continue
        normal = rows.get((id_, profile, 'normal'))
        if normal:
            current = rows[id_, profile, phase]
            r.require(current['estado'] == normal['estado'] and stable_observed(current.get('observed')) == stable_observed(normal.get('observed')),
                      'TRANSVERSAL_UNSTABLE', [id_, profile, phase])
    return r, {'counts': summaries, 'artifact_hashes_checked': len(data['artifacts']),
               'recognized_query_rejection_policy_sha256': digest(DIAG/'p1322-review-query-exit2-policy.md'),
               'semantic_limits': ['PDF subset font name difference alone is mechanics, not language debt.',
                                   'Raster exact-pixel difference does not by itself establish visible language divergence.',
                                   'Path spelling changes across outside-root fixtures require causal isolation before regression attribution.',
                                   'Artifact presence and exit status only certify the explicitly named smoke observable.']}


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('mode', choices=['preflight', 'runtime', 'gates', 'supplement', 'transversal', 'transversal-r2', 'query-controls', 'preservation'])
    parser.add_argument('--output')
    args = parser.parse_args()
    started = utc()
    t0 = time.monotonic()
    r, extra = {'preflight': lambda: (preflight(), {}), 'runtime': runtime_review,
                'gates': gate_review, 'supplement': supplement_review, 'transversal': transversal_review,
                'transversal-r2': lambda: transversal_review('-r2'), 'query-controls':query_rejection_controls,
                'preservation':preservation_review}[args.mode]()
    receipt = {'role': 'D', 'isolation': 'not technically attested', 'start': started,
               'end': utc(), 'seconds': time.monotonic()-t0,
               'checker_sha256': digest(__file__), 'plan_sha256': digest(DIAG/'p1335-review-plan.md'),
               'inputs': INPUTS,
    ...
    if args.output:
        ...
        if path.parent != DIAG or not path.name.startswith('p1335-review-') or path.exists():
            raise RuntimeError('Reviewer outputs must be new p1335-review-* artifacts')
        patch = '*** Begin Patch\n*** Add File: '+str(path)+'\n'+''.join(
