...
spec = importlib.util.spec_from_file_location('r', Path(__file__).with_name('p1327-record.py'))
r = importlib.util.module_from_spec(spec)
spec.loader.exec_module(r)
s = r.state()
r.verify(s)
m = json.loads((r.D / 'p1327-manifest.json').read_text())
rows = []
for owner, prompt_path in [(r.OWNER, r.PROMPT), (r.PAIRS[2], r.PAIRS[3])]:
    ...
    rows.append(dict(owner=owner, prompt=prompt_path, hash_a=a, hash_b=b, nucleus_pin=pin.hex()))
...
print(json.dumps(dict(pass_gate=True, rows=rows, manifest_sha256=r.sha(r.D / 'p1327-manifest.json')), indent=2))
