...
def save(path,value):
    ...
    if path.exists():
        patch='*** Begin Patch\n*** Update File: '+str(path)+'\n@@\n'+''.join('-'+x+'\n' for x in path.read_text().rstrip('\n').split('\n'))
...
def freeze(a):
    ...
    assert m['runner_sha256']==sha(D/'p1318-ab-runner-r0.py')
    assert m['writer_revision']['current_runner_sha256']==sha(__file__)
    assert m['cases_sha256']==sha(CASES) and m['l0_normative_sha256']==normative()
    ...
    files=[Path(__file__),CASES,Path(a.measurement),D/'p1318-ab-runner-r0.py',D/'p1318-ab-baseline-runs.json',D/'p1318-ab-writer-focal.json',D/'p1318-ab-writer-repair.py',D/'p1317-ab-cases.json',D/'p1317-ab-candidate-runs.json',D/'p1317-ab-freeze.json']
