...
def prepare(phase):
    ...
    if phase == "final":
        focal_path = BASE / (PREFIX + "focal-r1-observation.json")
        focal = read(focal_path)
        assert focal["focal_witness_valid"] is True
        inputs[str(focal_path.relative_to(ROOT))] = sha(focal_path)
    inputs[str((BASE / (PREFIX + "focal-manifest.json")).relative_to(ROOT))] = sha(BASE / (PREFIX + "focal-manifest.json"))
    inputs[str((BASE / (PREFIX + "focal-vanilla-runs.json")).relative_to(ROOT))] = sha(BASE / (PREFIX + "focal-vanilla-runs.json"))
    cases = []
    for original in prior["cases"]:
        if phase == "focal-r1" and original["id"] not in ("strong-static-match", "strong-static-miss"):
            continue
    ...
    for element in (["strong"] if phase == "focal-r1" else ["strong", "emph", "text"]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("phase", choices=("focal-r1", "final"))
