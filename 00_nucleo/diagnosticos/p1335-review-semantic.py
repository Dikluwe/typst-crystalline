...
spec = importlib.util.spec_from_file_location('dchecker', D/'p1335-review-check.py')
...
# Lower bounds remeasured in p1335-review-owner-evidence.md, not inherited ranks.
MINIMUM_OWNERS = {
    'loader-data-source-missing': {LOADING, DISPATCH},
    'loader-path-missing': {LOADING, DISPATCH},
    'module-import-error-trace': {'01_core/src/compiler/eval/modules.rs'},
    'math-lexical-callee-resolution': {'01_core/src/compiler/eval/math.rs'},
}
...
def table(name):
    ...
    return list(csv.DictReader(data.decode().splitlines(), delimiter='\t', quoting=csv.QUOTE_NONE))
...
def verify_transitions(transitions, current, previous):
    ...
    for row in current:
        ...
        if not t:
            continue
        prior = old.get(key)
        comparable = bool(prior and prior['expression'] == row['expression'])
        literal = comparable and all(prior[s][k] == row[s][k] for s in ('vanilla','crystalline') for k in ('exit_code','stdout_base64','stderr_base64'))
        if not prior:
            expected_transition = 'ADDED_FRESH'
        elif not comparable:
            expected_transition = 'NOT_COMPARABLE_SOURCE'
        elif literal:
            expected_transition = 'UNCHANGED_COMPLETE_CHANNELS'
        ...
        r.require(t.get('fixture_comparable') == str(comparable).lower() and t.get('complete_literal_equal') == str(literal).lower(), 'TRANSITION_LITERAL_MEASUREMENT', key)
        r.require(t['previous_runtime_class'] == (prior['runtime_class'] if prior else 'NOT_IN_P1322'), 'TRANSITION_PRIOR_CLASS', key)
...
def verify_metrics(summary, ledger, normal, previous, supplement):
    r = core.Review()
    ids = {x['id'] for x in normal}
    old_ids = {x['id'] for x in previous}
    counts = dict(collections.Counter(x['runtime_class'] for x in normal))
    closed = sum(x['runtime_class'] in core.MATCH for x in normal)
    principal = summary['principal']
    r.require(principal['probes']==len(ids) and principal['cells']==len(normal), 'METRIC_DENOMINATOR', principal['cells'])
    r.require(principal['raw_counts']==counts and principal['raw_closed_cells']==closed, 'METRIC_COUNTS', counts)
    r.require(abs(principal['raw_ratio']-closed/len(normal))<1e-14, 'METRIC_RAW_RATIO', principal['raw_ratio'])
    r.require(principal['unknown']==counts.get('EXECUTION_UNKNOWN',0), 'METRIC_UNKNOWN', principal['unknown'])
    extensions = {x['path'] for x in ledger if x['current_language_class']=='DOCUMENTED_PRODUCT_EXTENSION'}
    retained = [x for x in normal if x['path'] not in extensions]
    adjusted = summary['adjusted']
    r.require(set(adjusted['excluded_paths'])==extensions, 'METRIC_EXTENSION_EXCLUSIONS', sorted(extensions))
    r.require(adjusted['cells']==len(retained) and adjusted['raw_counts']==dict(collections.Counter(x['runtime_class'] for x in retained)), 'METRIC_ADJUSTED_DENOMINATOR',len(retained))
    r.require(abs(adjusted['closed_ratio']-sum(x['runtime_class'] in core.MATCH for x in retained)/len(retained))<1e-14, 'METRIC_ADJUSTED_RATIO', adjusted['closed_ratio'])
    temporal = summary['temporal']
    r.require(temporal['previous_probe_count']==len(old_ids) and temporal['current_historical_probe_count']==len(old_ids&ids), 'TEMPORAL_PRINCIPAL_ONLY', temporal)
    r.require(temporal['added_paths']==len(ids-old_ids), 'TEMPORAL_ADDED_PATHS', temporal['added_paths'])
    r.require(summary['supplement']['cells']==len(supplement) and summary['supplement']['cases']==len({x['id'] for x in supplement}), 'SUPPLEMENT_DENOMINATOR',summary['supplement']['cells'])
    r.require(summary['supplement']['outside_principal_denominator'] is True, 'SUPPLEMENT_INFLATION', summary['supplement'])
    return r


def run():
    r = core.preflight()
    catalog = core.read('p1335-probe-catalog.json')
    normal = core.read('p1335-matrix-normal.json')['results']
    previous = core.read('p1322-matrix-normal.json')['results']
    supplementary = core.read('p1335-sentinels-normal-r2.json')['rows']
    selection = core.read('p1335-classification-selection.json')
    ledger = table('p1335-classification-owner-ledger.tsv')
    pieces = [verify_principal(ledger, catalog, normal),
              verify_supplement(table('p1335-classification-supplemental-ledger.tsv'), supplementary),
              verify_transitions(table('p1335-classification-transitions.tsv'), normal, previous),
              verify_selection(selection),
              verify_metrics(core.read('p1335-classification-summary.json'), ledger, normal, previous, supplementary)]
    for piece in pieces:
        core.merge_review(r, piece)
    return {'at': core.utc(), 'checker_sha256': core.digest(__file__), 'base_checker_sha256': core.digest(D/'p1335-review-check.py'),
            'plan_sha256': core.digest(D/'p1335-review-plan.md'), 'inputs': core.INPUTS, **r.result()}
