...
BINARY_FIXTURE = Path('/tmp/p1319-ab-fixtures/data-lf.csv')
BINARIES = {
...
def classify(left, right, observation):
    if left.get('unknown') or right.get('unknown'):
        return 'UNKNOWN'
    if any(r['exit'] == 2 and 'Usage:' in r['stderr'] for r in (left, right)):
...
def cases():
    ...
    for route, source in [('str', '"data-lf.csv"'), ('path', 'path("data-lf.csv")')]:
        add('csv-binary-file-' + route, f'csv({source})', 'diagnostic')
        rows[-1]['cwd'] = str(BINARY_FIXTURE.parent)
    return rows
...
class ClassifierTests(unittest.TestCase):
    ...
    def test_usage_error_is_not_product_gap(self):
        self.assertEqual(classify(self.row('', 2, 'Usage: typst eval'), self.row('1'), 'diagnostic'), 'UNKNOWN_HARNESS')

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--output', required=True, type=Path)
    parser.add_argument('--self-test', action='store_true')
    parser.add_argument('--focal-only', action='store_true')
    ...
    def run_one(item):
        ...
        for side, (binary, _) in BINARIES.items():
            command = [binary, '--color=never', 'eval', case['expression'], '--format', 'json']
            if features:
                command += ['--features', ','.join(features)]
            observations[side] = invoke(command, Path(case.get('cwd', FIX)))
    ...
    compile_rows = {}
    for side, (binary, _) in BINARIES.items():
        compile_rows[side] = invoke([binary, '--color=never', 'compile', str(FIX / 'captured.typ'),
                                    str(tmp / (side + '-captured.svg')), '--format', 'svg'], FIX)
    matrix = []
    for profile in ([] if args.focal_only else ['default', 'html', 'a11y-extras']):
    ...
    receipt = dict(schema='p1320-parity-audit/v1', upstream='a51e02804', before=before,
        ...
        binary_fixture=dict(path=str(BINARY_FIXTURE), sha256=sha(BINARY_FIXTURE), bytes=list(BINARY_FIXTURE.read_bytes())),
        calibration='r1: global --color before subcommand; focal rerun only; r0 focal invalid, matrix retained',
