"""Close P1330 only when frozen gates and independent reviews agree."""
...
m = json.loads((D/'p1330-manifest-r2.json').read_text())
assert r.sha(D/'p1329-closure.json') == m['previous_closure_sha256']
assert r.sha(D/'p1330-baseline.json') == m['baseline_sha256']
...
gates = {}
for name in ['final-build','final-unit-green','workspace-tests','final-fmt-resealed','final-lint',
    ...
    assert receipt['manifest_sha256'] == r.sha(D/'p1330-manifest-r2.json'),name
    gates[name] = dict(path=str(path),sha256=r.sha(path),exit=0)
red = json.loads((D/'p1330-unit-red.json').read_text())
assert red['exit'] == 101 and 'FAILED' in red['stdout']
assert any(int(n)>0 for n in re.findall(r'running (\d+) tests',red['stdout']))
integration = json.loads((D/'p1330-test-integration.json').read_text())
assert all(r.sha(path) == h for path,h in integration['frozen'].items())
history = json.loads((D/'p1330-historical-expectations.json').read_text())
assert history['pass_gate'] and len(history['changed']) == 4 and history['unchanged'] == 248
assert history['expected_sha256'] == r.sha(D/'p1330-ab-cli-expected.json')
frozen = json.loads((D/'p1330-ab-cli-expected.json').read_text())
expected = {(x['case'],x['profile']):x['expected'] for x in frozen['expectations']}
assert len(expected) == len(frozen['expectations']) and len(expected)>252
binary_hash = r.sha(m['target']+'/release/typst')
for order in ['normal','repeat','reverse']:
    cli = json.loads((D/('p1330-ab-cli-'+order+'.json')).read_text())
    assert cli['binaries']['CANDIDATE']['sha256'] == binary_hash
    assert cli['manifest_sha256'] == r.sha(D/'p1330-manifest-r2.json')
...
report,review,ab_review = [D/('p1330-'+name) for name in ['final-report.md','review-final.md','ab-receipt.md']]
assert report.is_file() and 'PASS_SCOPED' in review.read_text()
assert 'PASS' in ab_review.read_text()
artifacts = {str(p):r.sha(p) for p in sorted(D.glob('p1330-*')) if p.is_file()}
r.save('closure',dict(at=r.now(),state=s,manifest_sha256=r.sha(D/'p1330-manifest-r2.json'),
    baseline_sha256=r.sha(D/'p1330-baseline.json'),previous_closure_sha256=r.sha(D/'p1329-closure.json'),
    ...
    binary_sha256=binary_hash,limits=['Only calc.abs Int overflow; no general parity',
