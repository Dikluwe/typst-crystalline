...
fn opaque_predicates(case: &Json, observed: &Json) -> Vec<String> {
    ...
    // Mechanical port of pinned independent assert_opaque, SHA2fd325da...
    // The external final collector also invokes that original Python function.
    if observed["private_discriminant"] != "Unproven" {
        failures.push("actual private opaque relation must be Unproven".into());
    }
    for key in ["validation", "validation_sinks"] {
        let keys = observed[key]
            .as_object()
            .map(|o| o.keys().map(String::as_str).collect::<Vec<_>>());
        if keys != Some(vec!["independent"]) {
            failures.push(format!("exact opaque channel key set {key}"));
        }
    }
    let mut channels = vec![
        ("construction_sink", &observed["construction_sink"]),
    ...
    if observed["validation"]["independent"]["result"] == "Err" {
        channels.push((
            "validation.independent.Err",
            &observed["validation"]["independent"]["diagnostics"],
        ));
    }
    if observed["nonconvergence_diagnostics"]["result"] == "Err" {
        channels.push((
            "nonconvergence_diagnostics.Err",
            &observed["nonconvergence_diagnostics"]["diagnostics"],
        ));
    }
    for (channel, diagnostics) in channels {
        let Some(diagnostics) = diagnostics.as_array() else {
            failures.push(format!("missing diagnostic vector {channel}"));
            continue;
        };
        for d in diagnostics {
            if d["severity"] != "Error" {
                failures.push(format!("opaque warning forbidden in {channel}"));
            }
            let Some(message) = d["message"].as_str() else {
                failures.push(format!("missing message {channel}"));
                continue;
            };
            let detail = message
                .strip_prefix("value of `")
                .and_then(|s| s.strip_suffix("` did not converge"));
            if message == "document did not converge within five attempts"
                || detail.is_some_and(|s| !s.contains('\n'))
            {
                failures.push(format!(
                    "nonconvergence message disguised as Err in {channel}"
                ));
            }
        }
    }
    for key in ["construction_sink", "body_sink"] {
        if observed[key] != json!([]) {
            failures.push(format!("opaque source has no ordinary side effects: {key}"));
        }
    }
    failures
}
...
pub(super) fn run_public_opaque_matrix(actual: ActualRelation, identity: &Json) {
    let fixture: Json = serde_json::from_str(include_str!(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../00_nucleo/diagnosticos/p1339-ab-batch1-public-opaque-fixture-r2.json"
    )))
    .unwrap();
