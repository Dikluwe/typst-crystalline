...
if __name__ == '__main__':
    ...
    parser.add_argument('--receipt', help='New canonical p1308-mutation-*.json receipt written using apply_patch')
    args = parser.parse_args()
    result = run(read(args.spec))
    rendered = json.dumps(result,ensure_ascii=False,indent=2)
    if args.receipt:
        destination = Path(args.receipt).resolve()
        assert destination.parent == HERE and destination.name.startswith('p1308-mutation-') and destination.suffix == '.json'
        assert not destination.exists(), 'Never overwrite an earlier receipt'
        patch = '*** Begin Patch\n*** Add File: ' + str(destination) + '\n' + '\n'.join('+' + line for line in rendered.splitlines()) + '\n*** End Patch\n'
        import subprocess
        subprocess.run(['apply_patch'], input=patch, text=True, capture_output=True, check=True)
        print(json.dumps({'receipt':str(destination),'sha256':sha(destination),'status':result['status'],'witnesses':result.get('witnesses'),'reason':result.get('reason')}))
    else:
        print(rendered)
