"""Replay immutable predecessors against P1312; only predeclared CSV deltas."""
...
def module(name, filename):
    ...
    return obj
r = module('record', 'p1313-record.py')
def read(name):
    return json.loads((D/name).read_text())
baseline = read('p1313-implementation-baseline.json')
...
if phase == 'p1310':
    ...
    expected = {(x['case'], x['profile']): x['observable'] for x in read('p1312-p1310-replay.json')['rows']}
    ...
    allowed = {('csv.wrong', p) for p in ab.PROFILES}
    inputs = ['p1310-ab-frozen-r2.json', 'p1310-ab-suite.py', 'p1310-ab-freeze-measurement.json', 'p1312-p1310-replay.json']
elif phase == 'p1311':
    ab = module('ab', 'p1311-ab-runner.py')
    expected = {(x['case'], x['profile']): x['observable'] for x in read('p1312-p1311-replay.json')['rows']}
    env = dict(os.environ)
    ...
    inputs = ['p1312-p1311-replay.json', 'p1311-ab-runner.py', 'p1311-ab-cases.json']
else:
...
for row in rows:
    ...
    row['p1312_observable'] = expected[key]
...
r.save(phase+'-replay', {'before': before, 'after': r.state(), 'candidate': candidate,
    ...
    'counts_against_p1312': {'Preserved': len(rows)-len(changed), 'Changed': len(changed), 'Unknown': unknown},
    'checks': checks, 'pass': all(checks.values()), 'seconds': time.monotonic()-tick,
    'policy': 'Historical oracles immutable. P1310 csv.wrong only changes to prepatch vanilla; P1311 wholly preserved against P1312. Historical L0 pins do not describe the explicitly approved P1313 amendment.'})
print(checks)
