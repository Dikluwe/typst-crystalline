...
| 5 | Walk fn 7 parâmetros | **CONFIRMADO** | `01_core/src/engine/introspect.rs:714` declara: `pub(crate) fn walk(content, locator, tags, intr, auto_label_counter, lang, label_from_parent)`. Sete parâmetros. Sig matches ADR-0070 §validação empírica (`5 params → 7 params (+intr +auto_label_counter +lang -state)`). |
| 6 | 2 loops fixpoint, MAX=5 | **CONFIRMADO** | `01_core/src/engine/introspect/fixpoint.rs:33`: `pub const MAX_FIXPOINT_ITERATIONS: usize = 5`. `01_core/src/engine/layout/mod.rs:1506`: `const MAX_ITERATIONS: usize = 5`. Dois loops paralelos, ambos cap 5. |
