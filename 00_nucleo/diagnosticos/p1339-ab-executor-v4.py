...
def checked_inputs(plan):
    ...
    assert sha(plan.get('contract_path',str(D/'p1339-contract.json')))==plan['contract_sha256'],'Contract identity drift'
    contract=read(plan.get('contract_path',str(D/'p1339-contract.json')))
    for name,digest in contract.get('l0_normative_sha256',{}).items():
        data=(ROOT/name).read_bytes();lines=data.splitlines(keepends=True)
        boundary=next((i for i,line in enumerate(lines) if line in (b'\n',b'\r\n')),len(lines))
        matches=[i for i,line in enumerate(lines[:boundary]) if re.fullmatch(rb'Hash do C\xc3\xb3digo: [0-9a-f]{8}\r?\n',line)]
        assert len(matches)==1,('Invalid canonical code-hash metadata',name)
        normative=b''.join(line for i,line in enumerate(lines) if i!=matches[0])
        assert hashlib.sha256(normative).hexdigest()==digest,('Normative L0 drift',name)
