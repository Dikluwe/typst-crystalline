...
def run(phase,ids=None):
    f=json.loads((D/'p1335-sentinels-freeze-r2.json').read_text());m.verify(f);corpus=json.loads((D/'p1335-sentinels-cases-r2.json').read_text());before=r.state();start=r.now()
    ...
    m.verify(f);r.save('sentinels-'+phase+'-r2',dict(at=start,end=r.now(),manifest_sha256=r.sha(D/'p1335-manifest.json'),freeze_sha256=r.sha(D/'p1335-sentinels-freeze-r2.json'),before=before,after=r.state(),binaries=f['binaries'],rows=rows,counts=dict(Counter(x['runtime_class'] for x in rows))))
