...
def observation_inputs():
    ...
    def visit(path):
        ...
        for m in re.finditer(r'"/\.\./(00_nucleo/diagnosticos/[^"\n]+)"', text):
            visit(m.group(1))
        for m in re.finditer(r'"(/src/[^"\n]+)"', text):
            visit('01_core'+m.group(1))
    for path in [
        '01_core/src/compiler/eval/mod.rs',
        '01_core/src/compiler/stdlib/primitives_constructors/array.rs',
        '00_nucleo/diagnosticos/p1339-verifier-array-successor-seal-r1.json',
        ...
        '00_nucleo/diagnosticos/p1339-seal.json',
    ]: visit(path)
    return {p:sha(ROOT/p) for p in sorted(seen)}
def state():
    def git(*args):
        return subprocess.check_output(['git',*args],cwd=ROOT,text=True)
    return dict(at=now(),head=git('rev-parse','HEAD').strip(),status=git('status','--short'),diff_stat=git('diff','HEAD','--stat'),
      protected_test_inputs=observation_inputs(),modified_sha256={p:sha(ROOT/p) for p in git('diff','HEAD','--name-only').splitlines()},
...
def main():
    ...
    result=dict(argv=argv,cwd=str(ROOT),start=at,end=now(),seconds=time.monotonic()-tick,
       ...
       before=before,after=after,source_unchanged=before['protected_test_inputs']==after['protected_test_inputs'] and before['head']==after['head'] and before['modified_sha256']==after['modified_sha256'] and before['untracked_product_sha256']==after['untracked_product_sha256'],
       ...
       interpretation='Raw product gate execution with recursive owner/style include pins; no independent verdict assigned')
