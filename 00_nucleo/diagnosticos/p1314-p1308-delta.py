"""Distinguish new P1314 CSV options from accepted P1310/P1311/P1312 deltas."""
...
names = ['p1314-p1308-replay.json', 'p1313-p1308-replay.json',
         'p1309-sentinels-p1308-vanilla.json', 'p1313-p1308-delta.json']
def read(name):
...
checks = {
    ...
    'candidate_matches_build': current['binary_sha256'] == read('p1314-build.json')['candidate']['sha256'],
}
r.save('p1308-delta', {
    'at': r.now(), 'state': r.state(), 'candidate': read('p1314-build.json')['candidate'],
    'inputs': {n: r.sha(D/n) for n in names}, 'runner_sha256': r.sha(__file__),
    'counts_against_frozen_predecessor': current['counts'],
    'preserved_against_p1313': len(c)-len(changed),
    ...
    'comparison': 'Exact frozen language envelopes; new deltas isolated against P1313. Old P1308 expectations remain immutable, not relabeled as a green replay.'})
