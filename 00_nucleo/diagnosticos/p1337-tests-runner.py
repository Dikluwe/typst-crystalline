"""Independent P1337 A/B runner. Frozen before C; no candidate source reads."""
import argparse
import base64
...
MANIFEST_SHA = "7c09595cde021b89fffa7c316b7706797ba3898aa5b6687cdfaf2deacb0e65ff"
...
def main():
    ...
    assert sha(D / "p1337-manifest-r1.json") == MANIFEST_SHA
    m, cases = read("p1337-manifest-r1.json"), read("p1337-tests-cases.json")
    started = state()
    hashes = {name: sha(D/name) for name in ["p1337-tests-cases.json", "p1337-tests-runner.py", "p1337-tests-module.rs.txt", "p1337-tests-local-patch.json", "p1337-tests-scope.md"]}
    if args.mode == "baseline":
        products = {"vanilla": m["vanilla"], "baseline": m["baseline_binary"]}
    else:
        freeze = read("p1337-tests-freeze.json")
        assert hashes == freeze["protected"]
        assert sha(D/"p1337-tests-baseline.json") == freeze["baseline_runs_sha256"]
        assert sha(D/"p1337-tests-expectations.json") == freeze["expectations_sha256"]
    ...
    if args.finalize_baseline:
        ...
        measured = read("p1337-tests-baseline.json")
    ...
    if not args.finalize_baseline:
        save("p1337-tests-" + args.mode + ".json", result)
    ...
    if args.mode == "baseline":
        ...
        save("p1337-tests-expectations.json", expectations)
        save("p1337-tests-freeze.json", {"at":utc(), "manifest_sha256":MANIFEST_SHA,"protected":hashes,
             "baseline_runs_sha256":sha(D/"p1337-tests-baseline.json"), "expectations_sha256":sha(D/"p1337-tests-expectations.json"),
             "regime":"A/B executed without technical isolation attestation; baseline source read only before freeze for local test integration",
             "rust_tests":"01_core/src/compiler/eval/bindings/field_access.rs::p1337_tests; frozen before implementation",
        ...
        expectations = {(e["id"],e["profile"]):e for e in read("p1337-tests-expectations.json")}
        ...
        save("p1337-tests-comparison.json", {"at":utc(),"manifest_sha256":MANIFEST_SHA,
             "candidate_runs_sha256":sha(D/"p1337-tests-candidate.json"),"freeze_sha256":sha(D/"p1337-tests-freeze.json"),"rows":comparisons})
