"""Independent source/argv/channels and legacy-envelope checks for R3 sentinels."""
import base64
import collections
...
def run(phases):
    r=c.preflight();corpus=c.read('p1335-sentinels-cases-r2.json');frozen=c.read('p1335-sentinels-freeze-r3.json')
    ...
    maps={};summaries={}
    for phase in phases:
        matrix=c.read('p1335-sentinels-'+phase+'-r3.json')
        r.require(matrix['freeze_sha256']==c.digest(D/'p1335-sentinels-freeze-r3.json'),'SUPPLEMENT_FREEZE',phase)
        ...
        for key,row in rows.items():
            ...
            for side in ('vanilla','crystalline'):
                ...
                if case['route']=='eval':
                    ...
                    if case['format'] in ('json','raw'):argv+=['--format',case['format']]
