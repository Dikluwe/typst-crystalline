"""Aggregate P1328 gates without weakening frozen expectations."""
...
s = r.state()
r.verify(s)
previous = json.loads((D / 'p1327-closure.json').read_text())
...
for name in ['final-build', 'unit-green', 'workspace-tests', 'final-fmt', 'final-lint',
    ...
    assert receipt['manifest_sha256'] == r.sha(D / 'p1328-manifest.json'), name
    gates[name] = dict(path=str(path), sha256=r.sha(path), exit=0)
red = json.loads((D / 'p1328-unit-red-r2.json').read_text())
assert red['exit'] == 101 and 'FAILED' in red['stdout']
assert any(int(n) > 0 for n in re.findall(r'running (\d+) tests', red['stdout']))
integration = json.loads((D / 'p1328-test-integration.json').read_text())
assert all(r.sha(path) == h for path, h in integration['frozen'].items())
integration_r2 = json.loads((D / 'p1328-test-integration-r2.json').read_text())
assert all(r.sha(path) == h for path, h in integration_r2['frozen'].items())
frozen = json.loads((D / 'p1328-ab-cli-expected-r1.json').read_text())
...
for order in ['normal', 'repeat', 'reverse']:
    cli = json.loads((D / ('p1328-ab-cli-' + order + '.json')).read_text())
    assert cli['binaries']['CANDIDATE']['sha256'] == binary_hash
    assert len(cli['cases']) == 112
    observed = {}
...
manifest = json.loads((D / 'p1328-manifest.json').read_text())
step_hash = r.sha(r.ROOT / '00_nucleo/materialization/typst-passo-1328.md')
assert step_hash == manifest['step_sha256']
report = D / 'p1328-final-report.md'
review = D / 'p1328-review-final.md'
assert report.is_file() and review.is_file() and 'PASS_SCOPED' in review.read_text()
ab_review = D / 'p1328-ab-receipt.md'
assert ab_review.is_file() and 'PASS' in ab_review.read_text()
artifacts = {}
for path in sorted(D.glob('p1328-*')):
    if path.is_file():
        artifacts[str(path)] = r.sha(path)
    elif path.is_dir():
        artifacts.update({str(p):r.sha(p) for p in sorted(path.rglob('*')) if p.is_file()})
r.save('closure', dict(at=r.now(), state=s, manifest_sha256=r.sha(D/'p1328-manifest.json'),
    baseline_sha256=r.sha(D/'p1328-baseline.json'), previous_closure_sha256=r.sha(D/'p1327-closure.json'),
    ...
    'Only Content diagnostic in calc.abs; no general calc parity',
    'External trace names, other types, numeric support/overflow and guards retain explicit baseline debts',
