...
- forma estática sobre zero, positivos, negativos, `-0.0`, inteiros e floats;
- equivalência semântica entre `angle.deg(x)` e `x * 1deg`, e entre
  `angle.rad(x)` e `x * 1rad`, sem exigir estrutura Rust igual;
- forma ligada sobre valores `Angle`, somente se o vanilla a expuser;
