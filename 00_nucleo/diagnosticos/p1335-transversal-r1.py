...
def warning_run(c,phase,f,data,tmp):
    ...
    return row
def run(focal=False):
    f=json.loads((D/'p1335-transversal-freeze-r1.json').read_text());m.verify(f);data=json.loads((D/'p1335-transversal-cases.json').read_text());before=r.state();at=r.now();tmp=Path(data['temp']);rows=[];locations=[];extra=[];warnings=[]
    ...
    m.verify(f);r.save('transversal-focal-r1' if focal else 'transversal-r1',dict(at=at,end=r.now(),manifest_sha256=r.sha(D/'p1335-manifest.json'),freeze_sha256=r.sha(D/'p1335-transversal-freeze-r1.json'),before=before,after=r.state(),binaries=f['binaries'],matrix=rows,location=locations,extra=extra,warnings=warnings,temp=str(tmp),artifacts={str(p):r.sha(p) for p in tmp.rglob('*') if p.is_file()}))
