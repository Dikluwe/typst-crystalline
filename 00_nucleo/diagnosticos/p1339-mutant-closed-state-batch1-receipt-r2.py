...
runner = D / 'p1339-mutant-closed-state-final-runner-r2.py'
...
extras = [
    ...
    'p1339-mutant-closed-state-batch1-receipt-r2.py',
    ...
    'p1339-mutant-closed-state-batch1-receipt.json',
...
old_fixture = json.loads((D / 'p1339-ab-batch1-retention-lifecycle-typed-r2.json').read_text())
new_fixture = json.loads((D / 'p1339-ab-batch1-retention-lifecycle-typed-r3.json').read_text())
assert old_fixture['cases'] == new_fixture['cases'], 'collector real inputs changed'
assert old_fixture['dto_schema'] == new_fixture['dto_schema'], 'collector DTO changed'
receipt = {
    'successor_cause': 'Same batch1: independently authored lifecycle typed-r3/checker-v3 and coverage-map-r2. Driver calls original v3, which calls v2 and predecessor. No expectation rewritten by adapter author.',
    'collector_preserved_reason': 'Exact cases and dto_schema equality r2/r3 checked by this metadata generator. Frozen collector continues constructing identical real inputs from pinned r2; external successor driver checks those observations with canonical r3/v3.',
...
output = D / 'p1339-mutant-closed-state-batch1-receipt-r2.json'
