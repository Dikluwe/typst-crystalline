...
rawpath=D/'p1339-ab-batch1-focal-runs.json';raw=json.loads(rawpath.read_text())
manifest=json.loads((D/'p1339-ab-batch1-manifest-r2.json').read_text())
accept=json.loads((D/'p1339-verifier-batch1-manifest-acceptance-r1.json').read_text())
assert accept['manifest']==pin('p1339-ab-batch1-manifest-r2.json')
assert accept['verdict']=='ACCEPT_FOCAL_BATCH1_ONLY'
assert raw['processes']==384 and len(raw['rows'])==384
derivedpath=D/'p1339-ab-batch1-causal-derived-expectations.json'
out=D/'p1339-ab-batch1-focal-evaluation.json'
...
argv=['python3',str(D/'p1339-ab-batch1-public-predicate-v2.py'),'--raw',str(rawpath),'--purpose','calibration','--ids',','.join(manifest['existing_execution_slice']['ids']),'--focal',str(rawpath),'--focal-sha256',sha(rawpath)]
derived=None
...
if len(actual_cells)!=384 or len(actual_cells)!=len(set(actual_cells)) or set(actual_cells)!=expected_cells:failures.append({'kind':'classification_cell_bijection','actual_count':len(actual_cells),'unique_count':len(set(actual_cells))})
...
data={'schema':'p1339-focal-evaluation-receipt-v1','author':'/root/p1319_tests','regime':'executado sem atestacao de isolamento','at':now(),
 'authority_manifest':pin('p1339-authority-manifest-r2.json'),'contract':pin('p1339-contract-r3.json'),'batch_manifest':pin('p1339-ab-batch1-manifest-r2.json'),'independent_authorization':pin('p1339-verifier-batch1-manifest-acceptance-r1.json'),
 ...
 'recipe':recipe['artifact'],'recipe_call':recipe_call,'derived_receipt':derived,'predicate':pin('p1339-ab-batch1-public-predicate-v2.py'),'predicate_predecessor':pin('p1339-ab-batch1-public-predicate.py'),'predicate_call':predicate_call,
 ...
 'independent_verdict':'PENDING; author receipt is not verifier seal or acceptance','no_exit_zero_shortcut':'Every classification parsed and checked against exact384cell set and declared control policy. Process exit alone never grants pass.',
 'budget':{'aggregate_batches_executed_since_redesign':1,'aggregate_batches_limit':2,'full_preseal_used':0,'full_preseal_limit':2,'automatic_retries':0,'canonical_RED':0,'candidate':0,'future_internal_executions':0},
 'cost_delta':'One authorized384process focal; one frozen recipe invocation and one classifier invocation. Any failures retained; no modified expectations or automatic retry.',
