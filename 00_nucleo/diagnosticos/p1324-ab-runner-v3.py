...
CASES = HERE / 'p1324-ab-cases-v3.json'
MANIFEST = HERE / 'p1324-manifest.json'
FROZEN = HERE / 'p1324-ab-freeze-v3.json'
...
def main():
    ...
    parser.add_argument('--ids', help='Explicit bounded calibration subset, comma-separated')
    options = parser.parse_args()
    spec = json.loads(CASES.read_text())
    if options.label != 'oracle' and not options.ids:
    ...
    if options.ids:
        assert options.label != 'candidate', 'candidate may not bypass full freeze validation'
        selected = options.ids.split(',')
        assert all(any(c['id'] == key for c in cases) for key in selected)
    ...
    for order in options.orders.split(','):
        ...
        for profile, features in spec['profiles'].items():
            for case in reversed(cases) if order == 'reverse' else cases:
                ...
                key = (case['id'], profile)
                expected = case.get('oracle_expect', case['expect']) if options.label == 'oracle' else case['expect']
                if expected == 'feature':
    ...
    if options.label == 'oracle' and not options.focal and not options.ids:
