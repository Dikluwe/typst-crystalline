...
spec=importlib.util.spec_from_file_location('previous',D/'p1332-record.py')
...
def save(name,data):
    path=D/('p1333-'+name+'.json');assert not path.exists(),path
...
def verify(s):
    b=json.loads((D/'p1333-baseline.json').read_text())['state']
...
def init():
    s=state();prior=json.loads((D/'p1332-closure.json').read_text())
    ...
    expressions=[]
    for value in ['[x]','"x"','true','none','2pt+3em','-9223372036854775807-1','-2','-2pt','decimal("-3.5")']:
        expressions += [f'calc.abs({value}, bad: 1)',f'calc.abs({value}, 2)',f'calc.abs(bad: 1, {value}, 2)']
    expressions += ['calc.abs()','calc.abs(bad: 1)','calc.abs(value: -1)','calc.abs(bad: 1,value: -1)',
      '{let a=calc.abs.with(); a()}','$std.calc.abs()$',
      'calc.abs(-1,bad:1,2)','calc.abs(-1,2,bad:1)',
      '{let a=calc.abs.with([x]); a(2,bad:1)}','{let a=calc.abs.with(bad:1); a("x",2)}',
      '{let a=arguments(false,2,bad:1); calc.abs(..a)}',
      'calc.abs(..(2pt+3em,2))','calc.abs(false,bad:panic("later"))',
      '{import std; calc.abs(false,2)}','calc.sqrt("x",2)',
      '{import calc: abs; $abs(-1)$}','calc.abs(-2)','calc.abs(-2pt)']
    rows=[]
    ...
    save('baseline',dict(**public,state=s,original_owner=(ROOT/OWNER).read_text(),original_prompt=(ROOT/PROMPT).read_text(),historical_preserved=history,previous_closure_sha256=sha(D/'p1332-closure.json')))
    save('baseline-public',dict(**public,baseline_sha256=sha(D/'p1333-baseline.json')))

def command(name,argv):
    before=state();verify(before);at=now();tick=time.monotonic()
    target=json.loads((D/'p1333-target.json').read_text())['target']
    ...
    save(name,dict(at=at,end=now(),seconds=time.monotonic()-tick,argv=argv,cwd=str(ROOT),target=target,before=before,after=after,exit=p.returncode,stdout=p.stdout,stderr=p.stderr,manifest_sha256=sha(D/'p1333-manifest.json'),binary=dict(path=str(binary),sha256=sha(binary)) if binary.exists() else None))
