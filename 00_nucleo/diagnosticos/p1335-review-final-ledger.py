...
def main():
    ...
    for g in tx['matrix']:
        for row in g['results']:
            entry=mapped[row['id'],g['profile'],g['phase'],'transversal'];r.require(entry['raw_channel_state']==row['channel_state'] and entry['projection_state']==row['estado'],'TRANSVERSAL_RAW_NOT_HIDDEN',[row['id'],g['profile'],g['phase']])
