...
// Descendant of the frozen API harness. Successor r2: instrumental E0499 fix only.
...
fn run_style_case(
    ...
    let mut row;
    {
        ...
        let mut current_file = {
            let mut setup_tracked = product_tracked.reborrow_mut();
        ...
        for op in case["operations"].as_array().unwrap() {
            ...
            let mut operation_tracked = if validation_phase {
                validation_tracked.reborrow_mut()
            } else {
                product_tracked.reborrow_mut()
            };
            let mut engine = Engine {
                world: &world,
                font_metrics: &metrics,
