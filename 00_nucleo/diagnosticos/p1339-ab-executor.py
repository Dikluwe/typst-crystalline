...
def execute(case,binary,product,profile,order,outdir,ordinal):
    ...
    try:
        ...
        code=p.returncode;stdout=p.stdout.decode('utf-8');stderr=p.stderr.decode('utf-8')
        execution='Observed' if code in (0,1) else 'UnknownProcessExit'
    except subprocess.TimeoutExpired as e:
        timeout=True;code=None;stdout=(e.stdout or b'').decode('utf-8',errors='replace');stderr=(e.stderr or b'').decode('utf-8',errors='replace');execution='UnknownTimeout'
    ...
    for order in orders:
        for profile in PROFILES:
            for c in reversed(cases) if order=='reverse' else cases:
                ...
                for product,(binary,digest) in binaries.items():tasks.append((c,binary,product,profile,order,outdir,len(tasks)))
