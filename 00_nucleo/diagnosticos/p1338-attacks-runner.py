"""Isolated P1338 mutations, immutable per-round evidence, no main-tree writes."""
...
def prepare(manifest,plan):
 ...
 base=Path(tempfile.mkdtemp(prefix='p1338-attacks-')); workspace=base/'workspace'; workspace.mkdir()
 ...
 marker='        Value::Array(arr) => match field {'
 ...
 publish(DIAG/'p1338-attacks-prepared.json',record); print(json.dumps({'base':str(base),'tests_sha256':record['tests_sha256'],'candidate_sha256':record['candidate_sha256']}),flush=True)
def run(cache):
 prepared=DIAG/'p1338-attacks-prepared.json'; r=json.loads(prepared.read_text()); base=Path(r['base']); workspace=Path(r['workspace']); target=base/'target'
 ...
 publish(DIAG/'p1338-attacks-cache-copy.json',{'at':at,'seconds':time.monotonic()-t,'argv':command,'exit':p.returncode,'stdout':p.stdout,'stderr':p.stderr,'manifest_sha256':r['manifest_sha256']})
 ...
 for item in r['mutants']:
  ...
  command=['cargo','test','--release','--config',CONFIG,'--locked','--offline','-p','typst-core','--lib','p1338_tests','--','--nocapture']
  ...
  publish(DIAG/('p1338-attacks-run-'+ident+'.json'),record); runs.append(record)
 ...
 publish(DIAG/'p1338-attacks-results.json',{'at':utc(),'manifest_sha256':r['manifest_sha256'],'runs':runs,'complete':len(runs)==5,'scope':'instrumental opt-level=0 for typst-core only; not release normal proof; no technical isolation attestation or refinement seal'})
