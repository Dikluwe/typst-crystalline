...
def verify_principal(ledger, catalog, current):
    ...
    for id_, entry in lookup.items():
        ...
        p = probes[id_]
        rows = grouped[id_]
        r.require(entry['language_observable'] == p['expression'], 'LEDGER_EXPRESSION_BINDING', id_)
...
def verify_supplement(ledger, current):
    ...
    for id_, entry in entries.items():
        rows = grouped[id_]
        r.require(entry['language_observable'] == rows[0]['expression'], 'FUNCTIONAL_SOURCE_BINDING', id_)
