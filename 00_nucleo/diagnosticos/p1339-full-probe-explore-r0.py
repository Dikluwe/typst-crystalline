...
def cases():
    ...
    for unit in ['deg','rad']:
        ...
        for val in ['0deg','-0deg','90deg','-90deg','0rad','-0rad','1rad','-1rad','(float("inf") * 1deg)','(float("nan") * 1rad)']:
            for form in ['static','bound']:
                ...
                add(f'{route}-{form}-{val}',route,f'{{ let x = {call}; repr((type(x), x, x == x, calc.is-infinite(1 / x))) }}','Angle to float; signed, units, nonfinite')
    ...
    for version in ['version(1,2,3)','version(1)','version()']:
        for index in [0,1,2,3,9,-1,-2,-3,-4,9223372036854775807,-9223372036854775808]:
            for form in ['static','bound']:
                call=f'version.at({version},{index})' if form=='static' else f'{version}.at({index})'
...
def main():
    ...
    for key in list(env):
        if key.startswith('TYPST_'):
            del env[key]
    names=[('vanilla','vanilla')] if stage=='explore' else [('vanilla','vanilla'),('crystalline-before','candidate')]
    for name,key in names:
        rows=[]
        orders=['normal'] if stage=='explore' else ['normal','reverse']
        start=now()
        for order in orders:
            for n,case in enumerate(corpus if order=='normal' else list(reversed(corpus))):
                for profile,flags in (list(PROFILES.items())[:1] if stage=='explore' else PROFILES.items()):
