...
| **② F-realização** (gatilho disparado em F-3 inc-2 S3) | **Restam caso 1 (composição) + caso 3 (show-set, `Transformation::Style`)** — multi-passe `StyledElem`-scoped (`lab/.../content/mod.rs:744-752`). **Caso 4 (escopo): FECHADO no P340** — `[]` e `{}` **ambos confinam** (paridade com vanilla; `f3s3_caso4_...` `tests.rs:480`). A redação anterior ("`[]` vaza") era a descrição do **bug pré-P340** — **corrigida** (P351 Fase A). **Caso 2 (recursão): fechado** (P342–P350c). | parcial: casos 2 e 4 feitos | casos 1/3: **medir** sítios + testes na Fase A do lote (não assumir — P351 mostrou que a contagem "20 (1/18/1)" era da F-realização inteira, pré-fechos) |
