# Achados adiados na cadeia P700-730 — lista de controlo

## Por resolver

| Achado | Onde foi encontrado | Prioridade | Estado |
|---|---|---|---|
| Módulos stdlib expostos como `dictionary` em vez de `module` — `type(calc)` → `dictionary` no cristalino, `module` no vanilla (medido); `#import calc: min, max` → erro "import: a fonte tem de ser um caminho string ou um módulo, recebeu dictionary" (`01_core/src/rules/eval/modules.rs:187` exige `Value::Module`). Caso mínimo sem cetz: `#import calc: min, max` + `#(min(1, 2))`. É o **bloqueio actual** do cetz — com `array.slice` corrigido (P730), `line((0,0),(2,1))` avança até ao bounds via `aabb.typ:18` (`import calc: min, max` em corpo de função). Outros candidatos a verificar no mesmo passo: `sys`, `math`, restantes bindings stdlib | P730 (validação cetz) | Alta — bloqueio actual do diff de pixels do cetz; candidato natural a P731 | Aberto |
