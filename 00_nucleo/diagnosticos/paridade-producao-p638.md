...
| 3 | `01_core/src/engine/eval/mod.rs:819` — catch-all `eval_expr` engole `#break`/`#continue`/`#return` | `typst-eval/src/code.rs:74-154` (match exaustivo) + `flow.rs:26-36` + `lib.rs:88-91` | **Não** | Erros claros: `cannot break outside of loop`, etc. |
| 4 | `01_core/src/engine/eval/from_tags.rs:64` — `state.update(func)` descarta `Err` do callback | `typst-library/src/introspection/state.rs:498-504` (`func.call(...)?`) | **Não** | Erro do callback propagado (testado: divisão por zero) |
| 5 | `01_core/src/engine/eval/bibliography.rs:146` — entrada sem key/título omitida | `typst-library/src/model/bibliography.rs:377-381` | **Não** | Chave vazia → erro `bibliography contains entry with empty key`. Sem título → aceite (renderiza degradado, mas não omitido). |
| 6 | `01_core/src/engine/layout/bib_csl.rs:214` — `bib_entry_to_hayagriva` omite entrada por YAML/chave ausente | `typst-library/src/model/bibliography.rs:425-444` (`decode_library`) | **Não** | Erros de YAML propagados (`map_err(format_yaml_error)`). Chave vazia → erro. |
| 7 | `01_core/src/engine/layout/grid.rs:322` — `place_cells(...).unwrap_or_default()` descarta erro de grid inválida | Não existe `place_cells`; layout em `typst-layout/src/grid/` | **Sim (observable)** | Grid com células extra, colspan fora de limites ou conflitos de placement compila sem erro. Mecanismo diferente do cristalino (não detecta vs. detecta e descarta), mas o utilizador não é avisado. |
| 8 | `01_core/src/engine/eval/rules.rs:678` — `#set math.equation(numbering: <não-definido>)` silenciado por `.ok()` | `typst-eval/src/engine.rs:11-34` delega para `target.set(...)` | **Não** | Erro: `unknown variable: nao_existe` |
| 9 | `01_core/src/engine/eval/rules.rs:695` — `#set math.equation(numbering: <não-Str>)` ignorado | `EquationElem::set` via cast tipado | **Não** | Erro: `expected string, function, or none, found int` |
| 10 | `01_core/src/engine/eval/rules.rs:819` — `#set figure(numbering: <não-Str>)` ignorado | `FigureElem::set` via cast tipado | **Não** | Erro: `expected string, function, or none, found int` |
| 11 | `01_core/src/engine/eval/rules.rs:848` — `#set table(numbering: <não-Str>)` ignorado | `TableElem::set` via cast tipado | **Não** | Erro: `unexpected argument: numbering` (table vanilla não tem `numbering`) |
| 12 | `01_core/src/engine/eval/rules.rs:772` — `#set page(numbering: <não-Str/None>)` convertido silenciosamente | `PageElem::set` via cast tipado | **Não** | Erro: `expected string, function, or none, found int` |
| 13 | `01_core/src/engine/eval/rules.rs:785` — `#set page(columns: <não-Int>)` ignorado | `PageElem::set` via cast tipado | **Não** | Erro: `expected integer, found string` |
| 14 | `01_core/src/engine/eval/rules.rs:967` — `#set text(weight: <não-Int>)` ignorado | `TextElem::set` via cast tipado | **Não** | Erro: `expected integer, "thin", ...` |
| 15 | `01_core/src/engine/eval/rules.rs:706` — `#set document(title: 123)` silencioso | `DocumentElem::set` via cast tipado | **Não** | Erro: `expected content or none, found integer` |
| 16 | `01_core/src/engine/eval/rules.rs:746` — `#set page(width: "foo")` mantém dimensão anterior | `PageElem::set` via cast tipado | **Não** | Erro: `expected length or auto, found string` |
| 17 | `01_core/src/engine/stdlib/layout.rs:161` — `parse_track_sizing` devolve `None`; `grid(columns: "foo")` vira `auto` | `GridElem::set` via cast tipado | **Não** | Erro: `expected auto, relative length, fraction, integer, or array, found string` |
| 18 | `01_core/src/engine/eval/bindings.rs:141` — argumento posicional não-Str de `counter.display` ignorado | `Counter::display` em `typst-library/src/introspection/counter.rs:379-443` | **Não** | Erro: `expected string, function, or auto, found integer` |
| 19 | `01_core/src/engine/eval/bindings.rs:156` — `at:` inválido em `counter.display` ignorado | `Counter::display` via `Smart<LocatableSelector>` | **Não** | Erro: `expected label, function, location, selector, or auto, found integer` |
| 20 | `01_core/src/engine/eval/bindings.rs:164` — argumentos não reconhecidos em `counter.display` ignorados | `Counter::display` só aceita argumentos declarados | **Não** | Typos/argumentos extra dão erro de argumento inesperado |
| 21 | `01_core/src/engine/eval/bindings.rs:273` — equivalente a 19 no despacho sobre `Value::Counter` | `Counter::display` | **Não** | Mesmo erro que 19 |
| 22 | `01_core/src/engine/eval/bindings.rs:280` — equivalente a 18 no despacho sobre `Value::Counter` | `Counter::display` | **Não** | Mesmo erro que 18 |
| 23 | `01_core/src/engine/eval/bindings.rs:284` — argumento posicional ignorado quando pattern já definido | `Counter::display` | **Não** | Argumentos posicionais extra dão erro |
