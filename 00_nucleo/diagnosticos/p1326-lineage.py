"""Check P1326 reciprocal lineage and immutable normative/test inputs."""
import hashlib
...
spec = importlib.util.spec_from_file_location('record', Path(__file__).with_name('p1326-record.py'))
...
manifest = json.loads((r.D / 'p1326-manifest.json').read_text())
...
snippets = list(r.D.glob('p1326-ab-tests-r1.rs'))
assert len(snippets) == 1, snippets
snippet = snippets[0].read_bytes().strip()
assert source.count(snippet) == 1
legacy = r.D / 'p1326-ab-legacy-successor.rs'
assert source.count(legacy.read_bytes().strip()) == 1
after = r.state()
assert s['product_inventory'] == after['product_inventory']
print(json.dumps(dict(hash_a=a, hash_b=b, normative_sha256=hashlib.sha256(norm).hexdigest(), snippet_sha256=r.sha(snippets[0]), nucleus_pin=pin.hex(), manifest_sha256=r.sha(r.D / 'p1326-manifest.json'), pass_gate=True), indent=2))
