...
def enumerate_side(args):
    ...
    for profile in ['default', 'html']:
        ...
        if args.side == 'vanilla':
            ...
            argv.extend([str(OUT / f'p1322-inventory-vanilla-structural-input-{profile}.json'), profile, 'lab/surface-inventory/extra_seeds.json', args.binary])
        ...
        publish(f'p1322-inventory-{args.side}-{profile}{args.suffix}.json', payload)
        print(args.side, profile, len(payload['entries']), flush=True)

def observe(args):
    assert sha(args.binary) == args.expected_sha
    for profile in ['default', 'html']:
        source = OUT / f'p1322-inventory-{args.side}-{profile}{args.suffix}.json'
        ...
        assert sha(args.binary) == args.expected_sha
        publish(f'p1322-inventory-observables-{args.side}-{profile}{args.suffix}.json', result)
...
def catalog(args):
    ...
    for side in ['vanilla', 'crystalline']:
        for profile in ['default', 'html']:
            suffix = '-r2' if side == 'crystalline' else ''
            path = OUT / f'p1322-inventory-{side}-{profile}{suffix}.json'
            obs_path = OUT / f'p1322-inventory-observables-{side}-{profile}{suffix}.json'
