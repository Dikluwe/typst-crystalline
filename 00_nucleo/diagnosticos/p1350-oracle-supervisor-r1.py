#!/usr/bin/env python3
"""P1350 mandatory per-case supervisor and parent-owned causal journal."""
...
class RunJournal:
    ...
    def __init__(self, fd: int, run_id: str, expected_cases: int):
        if type(fd) is not int or fd < 0 or type(run_id) is not str or not run_id or type(expected_cases) is not int or expected_cases < 1:
    ...
    def finish(self) -> str:
        if self.next_order != self.expected_cases:
...
def validate_route_map(root: Path, plan: AdapterPlan, exact_ids: list[str]) -> dict[str, Any]:
    value = strict_json(_read_pinned(root, plan.route_map_path, plan.route_map_sha256), "route map", "AUTHORITY_UNAVAILABLE")
    if type(value) is not dict or list(value) != ["bindings", "closed_world", "schema"] or value["schema"] != "p1350-internal-route-map-r1":
        raise SupervisorFailure("SUPERVISOR_AUTHORITY", "route map schema")
    bindings = value["bindings"]
    if type(bindings) is not list or len(bindings) != 60 or len(exact_ids) != 60 or len(set(exact_ids)) != 60:
        raise SupervisorFailure("SUPERVISOR_AUTHORITY", "route coverage cardinality")
    seen: set[str] = set(); inherited = local = 0
    for row in bindings:
        if type(row) is not dict or list(row) != ["case_id", "operation", "route"] or row["route"] not in ("INHERITED_P1348_P1347_P1346", "P1350_LOCAL") or row["case_id"] in seen:
    ...
    if seen != set(exact_ids) or inherited != 48 or local != 12:
...
def _child_main(root: Path, case: dict[str, Any], plan: AdapterPlan, boot_r: int, ctl_w: int, ack_r: int, out_w: int, err_w: int) -> None:
    try:
        ...
        matches = [row for row in route_map["bindings"] if row["case_id"] == case["case_id"] and row["operation"] == case["operation"]]
        if len(matches) != 1:
        ...
        entry = getattr(adapter, "execute_in_executor", None)
        ...
        result = entry(dict(case), dict(matches[0]), context)
    ...
    try:
        ...
        if result_raw and len(result_raw) <= MAX_FRAME and not stderr_raw:
            try:
                ...
                if type(value) is not dict or value.get("schema") not in ("p1349-classification-result-r1", "p1349-integration-blocker-r1"):
