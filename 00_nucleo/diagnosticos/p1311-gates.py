...
spec = importlib.util.spec_from_file_location('p1311_record', D/'p1311-record.py')
r = importlib.util.module_from_spec(spec)
spec.loader.exec_module(r)

def read(name):
    return json.loads((D/('p1311-'+name+'.json')).read_text())
...
prior_changes = [p for p, h in baseline['prior_artifacts'].items()
                 if not (r.ROOT/p).is_file() or r.sha(r.ROOT/p) != h]
allowed = {'01_core/src/compiler/eval/bindings/field_access.rs', '00_nucleo/prompts/compiler/eval/bindings/field_access.md'}
...
outside = [line for line in state['status'].splitlines()
           ...
           and not line[3:].startswith('00_nucleo/diagnosticos/p1311-')
           and line[3:] != '00_nucleo/materialization/typst-passo-1311.md']
code = (r.ROOT/'01_core/src/compiler/eval/bindings/field_access.rs').read_bytes()
...
l0 = (r.ROOT/'00_nucleo/prompts/compiler/eval/bindings/field_access.md').read_text()
...
checks = {
    ...
    'p1310_preserved': read('p1310-replay')['counts'] == {'Preserved': 1036, 'Violated': 0, 'Unknown': 0},
...
r.save('gates', {'at': r.now(), 'state': state, 'checks': checks, 'pass': all(checks.values()),
    'baseline_sha256': r.sha(D/'p1311-baseline.json'), 'runner_sha256': r.sha(__file__),
    'inputs': {'p1311-'+name+'.json': r.sha(D/('p1311-'+name+'.json')) for name in receipts},
