...
spec=importlib.util.spec_from_file_location('matrix',D/'p1335-matrix.py')
...
def exact_orders(prefix, field):
    phases={phase:read(prefix+'-'+phase+('-r3' if prefix=='sentinels' else '')) for phase in ['normal','repeat','reverse']}
    ...
    return dict(pairs=len(maps['normal']),counts=counts,deltas=deltas,stable=not any(deltas.values()),
        inputs={str(D/('p1335-'+prefix+'-'+phase+('-r3' if prefix=='sentinels' else '')+'.json')):r.sha(D/('p1335-'+prefix+'-'+phase+('-r3' if prefix=='sentinels' else '')+'.json')) for phase in phases})
...
def main():
    ...
    historical=json.loads((D/'p1322-matrix-normal.json').read_text())
    ...
    r.save('aggregate',dict(at=r.now(),before=before,after=r.state(),manifest_sha256=r.sha(D/'p1335-manifest.json'),
        ...
        historical_transition_source=dict(path=str(D/'p1322-matrix-normal.json'),sha256=r.sha(D/'p1322-matrix-normal.json')),
