...
for key,value in {(x['case'],x['profile']):x for x in read('ab-cross-baseline')['cases']}.items():
    assert key not in baseline
    baseline[key]=value
for key,value in {(x['case'],x['profile']):x for x in read('ab-cross-cli-normal')['cases']}.items():
...
tests={}
for name in ['unit-red-r1','unit-green','workspace-tests']:
    g=read(name)
    ...
    tests[name].update(at=g['at'],end=g['end'],sha256=r.sha(D/('p1334-'+name+'.json')))
r.save('metrics',dict(at=r.now(),state=r.state(),manifest_sha256=r.sha(D/'p1334-manifest.json'),
    counts=dict(counts),remaining_cases=remaining,changed=changed,tests=tests,
    inputs={str(D/('p1334-'+n+'.json')):r.sha(D/('p1334-'+n+'.json')) for n in ['ab-cli-baseline','ab-cli-normal','ab-cross-baseline','ab-cross-cli-normal','unit-red-r1','unit-green','workspace-tests']}))
