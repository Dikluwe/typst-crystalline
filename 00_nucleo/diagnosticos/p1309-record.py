...
TARGET = "/dev/shm/p1309-target.PujwZQ"
...
def save(name, data):
    ...
    subprocess.run(["apply_patch", patch], cwd=ROOT, check=True, capture_output=True)
...
def init():
    ...
    save("baseline",{"schema":"p1309-baseline-v1","at":now(),"local":datetime.datetime.now().astimezone().isoformat(),
    ...
    save("manifest",{"schema":"p1309-manifest-v1","at":now(),"baseline_sha256":sha(D/"p1309-baseline.json"),
...
def command(name,argv):
    ...
    data={"schema":"p1309-command-v1","at":start,"end":now(),"seconds":time.monotonic()-tick,"argv":argv,"cwd":str(ROOT),"env":env,
          ...
          "manifest_sha256":sha(D/"p1309-manifest.json"),"recorder_sha256":sha(__file__)}
