...
old = json.loads((D/'p1330-ab-cli-expected.json').read_text())
new = json.loads((D/'p1331-ab-cli-expected-r2.json').read_text())
old_rows = json.loads((D/'p1330-ab-cli-normal.json').read_text())['cases']
new_rows = json.loads((D/'p1331-ab-cli-baseline.json').read_text())['cases']
...
assert len(old_expected) == len(old['expectations']) == 332
...
for k,e in old_expected.items():
    ...
    if e != new_expected[k]:
        assert k[0] in ('string','symbol')
        assert new_expected[k] == observed(new_map[k]['results']['VANILLA'])
        changed.append(k)
assert len(changed) == 8
r.save('historical-expectations-r2',dict(at=r.now(),pass_gate=True,unchanged=324,
    changed=changed,new_observations=len(new_expected)-332,
    expected_sha256=r.sha(D/'p1331-ab-cli-expected-r2.json'),
    previous_expected_sha256=r.sha(D/'p1330-ab-cli-expected.json'),
    baseline_sha256=r.sha(D/'p1331-ab-cli-baseline.json'),
    manifest_sha256=r.sha(D/'p1331-manifest-r2.json')))
