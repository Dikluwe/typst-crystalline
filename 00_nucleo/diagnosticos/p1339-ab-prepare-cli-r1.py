...
def write(p,s):
    assert p.parent==D and p.name.startswith('p1339-ab-') and not p.exists(),str(p)
    assert s.endswith('\n')
    patch='*** Begin Patch\n*** Add File: '+str(p)+'\n'+''.join('+'+l+'\n' for l in s[:-1].split('\n'))+'*** End Patch\n'
...
for group in contract['supplement_sets']:
    ...
    for c in raw:
        ...
        command=c.get('command',row.get('command',row.get('argv',[])));mode='eval' if c.get('expression') and (not command or command[1]=='eval') else 'compile' if group['id']=='context-dependency' else 'query'
        ...
        fullid=group['id']+'-'+cid
        if mode=='eval':add(fullid,'eval',policy,group['obligation'].split(','),expression=c['expression'],adapter='eval_exact',ancestor=ancestor,reference=dict(kind='fresh'))
...
write(D/'p1339-ab-cli-plan-r1.json',json.dumps(plan,ensure_ascii=True,indent=2)+'\n')
