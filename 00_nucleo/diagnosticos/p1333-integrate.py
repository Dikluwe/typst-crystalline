...
pairs=[('p1332-ab-p1328-successor.rs','p1333-ab-p1328-successor.rs'),
       ('p1332-ab-p1329-successor.rs','p1333-ab-p1329-successor.rs'),
       ('p1332-ab-p1330-successor.rs','p1333-ab-p1330-successor.rs'),
       ('p1332-ab-p1331-successor.rs','p1333-ab-p1331-successor.rs'),
       ('p1332-ab-tests.rs','p1333-ab-p1332-successor.rs')]
freeze=json.loads((D/'p1333-ab-native-freeze.json').read_text())
assert all(r.sha(D/name)==h for name,h in {**freeze['artifacts'],**freeze['inputs']}.items())
frozen={str(D/name):h for name,h in freeze['artifacts'].items()}
frozen[str(D/'p1333-ab-native-freeze.json')]=r.sha(D/'p1333-ab-native-freeze.json')
wanted=source
...
snippet=(D/'p1333-ab-tests.rs').read_text().strip()
...
r.save('test-integration',dict(at=r.now(),before=before,after=after,
    manifest_sha256=r.sha(D/'p1333-manifest.json'),frozen=frozen,
    integrated_snippets=[new for old,new in pairs]+['p1333-ab-tests.rs'],
