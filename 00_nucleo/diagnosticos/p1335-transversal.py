...
def warning_run(c,phase,f,data,tmp):
    ...
    for side in roles:
        ...
        row['observations'][side]=dict(command=argv,cwd=str(r.ROOT),started_at=at,duration_seconds=time.monotonic()-tick,binary_sha256=f['binaries'][side]['sha256'],stdin_base64=c['stdin_base64'],exit_code=code,stdout=out,stderr=err,complete=known,artifact=dict(path=str(output),sha256=r.sha(output)) if output.exists() else None,
            warning_contract=(err.startswith(fixed) and (not c['error_expected'] or err[len(fixed):].startswith('error:'))) if c['warning_expected'] else 'warning: HTML export is under active development' not in err)
    return row
def run(focal=False):
    f=json.loads((D/'p1335-transversal-freeze.json').read_text());m.verify(f);data=json.loads((D/'p1335-transversal-cases.json').read_text());before=r.state();at=r.now();tmp=Path(data['temp']);rows=[];locations=[];extra=[];warnings=[]
    for phase in (['focal'] if focal else ['normal','repeat','reverse']):
        ...
        for p in PROFILES:
            result=[pair(c,p,f['binaries'],tmp/phase/p/c['id'],tmp,reverse) for c in cases]
            ...
            if not focal:
                for c in cases:
                    if c['id'] in ['P1138-S-001','P1138-S-002','P1138-S-003']:
                        o=pair(c,p,f['binaries'],tmp/'location'/phase/p/c['id'],Path(data['internal']),reverse);o.update(profile=p,phase=phase);locations.append(o)
        ...
        for c in (list(reversed(selected)) if reverse else selected):warnings.append(warning_run(c,phase,f,data,tmp))
    m.verify(f);r.save('transversal-focal' if focal else 'transversal',dict(at=at,end=r.now(),manifest_sha256=r.sha(D/'p1335-manifest.json'),freeze_sha256=r.sha(D/'p1335-transversal-freeze.json'),before=before,after=r.state(),binaries=f['binaries'],matrix=rows,location=locations,extra=extra,warnings=warnings,temp=str(tmp),artifacts={str(p):r.sha(p) for p in tmp.rglob('*') if p.is_file()}))
