"""Fresh P1322 observations; retain the P1309 raw-channel projection."""
import argparse
...
def freeze():
    build=json.loads((D/'p1322-build.json').read_text())
    catalog=json.loads((D/'p1322-probe-catalog.json').read_text())
    historical=json.loads((D/'p1309-probe-catalog.json').read_text())
    ...
    inputs=['p1322-probe-catalog.json','p1309-probe-catalog.json','p1322-build.json',
        'p1322-manifest.json','p1322-record.py','p1322-matrix.py','p1309-matrix.py','p1309-record-r2.py']
    r.save('runtime-freeze',dict(at=r.now(),manifest_sha256=r.sha(D/'p1322-manifest.json'),
...
def run(phase):
    frozen=json.loads((D/'p1322-runtime-freeze.json').read_text());verify(frozen)
    catalog=json.loads((D/'p1322-probe-catalog.json').read_text())
    ...
    r.save('matrix-'+phase,dict(schema='p1322-global-matrix-v1',at=start,end=r.now(),seconds=time.monotonic()-tick,
        manifest_sha256=r.sha(D/'p1322-manifest.json'),freeze_sha256=r.sha(D/'p1322-runtime-freeze.json'),
        catalog_sha256=r.sha(D/'p1322-probe-catalog.json'),runner_sha256=r.sha(__file__),
