...
def read(name):
    return json.loads((D/('p1314-'+name+'.json')).read_text())
baseline = read('baseline')
names = ('unit-red', 'unit-green', 'build', 'workspace-tests', 'lint', 'fmt', 'diff-check', 'lineage-preflight', 'lineage-preview-final')
...
outside = [line for line in state['status'].splitlines() if line[3:] not in allowed | before_paths
    and not line[3:].startswith('00_nucleo/diagnosticos/p1314-')
    and line[3:] != '00_nucleo/materialization/typst-passo-1314.md']
...
checks = {
    'red_real': receipts['unit-red']['exit'] == 101 and '2 passed; 4 failed;' in receipts['unit-red']['stdout'],
    'all_final_green_commands': all(v['exit'] == 0 for k,v in receipts.items() if k != 'unit-red'),
    ...
    'p1313_preservation': read('p1313-replay')['pass'],
...
r.save('gates', {'at': r.now(), 'state': state, 'checks': checks, 'pass': all(checks.values()),
    ...
    'inputs': {'p1314-'+n+'.json': r.sha(D/('p1314-'+n+'.json')) for n in receipts},
