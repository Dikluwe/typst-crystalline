#!/usr/bin/env python3
"""Final R2 parent-owned transport oracle; legacy probe wire corrected."""
...
def validate_authorship_receipt(receipt_sha256: str, checker_sha256: str, root_sha256: str) -> dict[str, Any]:
    ...
    if receipt.get("schema") != "p1347-oracle-authorship-receipt-r2" or receipt.get("authoring_root_sha256") != root_sha256 or root_sha256 != authoring_root(checker_sha256):
...
def legacy_probe_request(challenge: bytes, nonce: bytes) -> bytes:
    """Exact inherited P1345 wire order; generic canonical sorting is forbidden here."""
    return (b'{"schema":"p1345-opaque-probe-request-r2","fresh_challenge_hex":"' + challenge.hex().encode() + b'","invocation_nonce_hex":"' + nonce.hex().encode() + b'"}\n')


def legacy_probe_output(value: dict[str, Any]) -> bytes:
    """Exact inherited P1345 primitive order, with one trailing LF."""
...
def probe_payload(raw: bytes, challenge: bytes, nonce: bytes) -> tuple[str, str]:
    try:
        ...
        exact_keys(value, sorted(PROBE_KEYS), "probe output")
        if list(value) != PROBE_KEYS or legacy_probe_output(value) != raw or value["schema"] != "p1345-opaque-probe-primitive-output-r2": raise Failure("SCHEMA", "probe wire")
...
def trace_probe(executable: bytes, operation: str = "canonical") -> tuple[str, str, dict[str, Any]]:
    ...
    try:
        ...
        challenge = secrets.token_bytes(32); nonce = secrets.token_bytes(32)
        request = legacy_probe_request(challenge, nonce)
...
def run_focal(checker_sha256: str, receipt_sha256: str, root_sha256: str) -> dict[str, Any]:
    ...
    return {"authoring_root_sha256": root_sha256, "attacks": first_records, "closed_world": {"rule": "P1347 transport focal only; no full, RED, candidate, seal or general equivalence."}, "controls": controls, "cost": {"elapsed_ms": elapsed_ms, "focal_revision": 2, "full_corpus_runs": 0, "process_runs": len(corpus["attacks"]) * 3 + 8}, "delta": {"p1346_survivors_replayed": 5, "p1346_survivors_remaining": sum(item["case_id"].startswith("P1346-") and item["actual"] != "Violated" for item in first_records), "transport_cases_added": len(first_records) - 5}, "determinism": {"normal_projection_sha256": projections[0], "repeat_projection_sha256": projections[1], "reverse_projection_sha256": projections[2], "stable": deterministic}, "execution": {"baseline_control_process": baseline_evidence, "candidate_read_or_executed": False, "full_corpus_runs": 0, "worker_parent_evidence": worker_parent_evidence}, "regime": "executado sem atestacao de isolamento", "revision": 2, "schema": "p1347-oracle-focal-report-r2", "step": 1347, "summary": {"correctly_violated": len(first_records) - len(survivors), "mutation_score": score, "positive_control_regressions": regressions, "survivors": survivors, "transport_unknown_count": sum(item["actual"] == "Unknown" for item in first_records), "valid_negatives": len(first_records)}, "verdict": "FOCAL_AUTHORED_R2_NOT_VERIFIED_NOT_SEALED" if not survivors and not regressions and deterministic and probe_actual == "Unknown" else "FOCAL_R2_BLOCKED_NOT_SEALED"}
...
def main() -> int:
    ...
    try:
        ...
        sys.stdout.buffer.write(canonical(report)); return 0 if report["verdict"] == "FOCAL_AUTHORED_R2_NOT_VERIFIED_NOT_SEALED" else 1
    except Failure as exc: sys.stderr.write(f"{exc.code}: {exc.detail}\n"); return 2
