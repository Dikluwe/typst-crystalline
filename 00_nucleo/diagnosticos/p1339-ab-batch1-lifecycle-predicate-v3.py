...
P = Path(__file__).with_name('p1339-ab-batch1-lifecycle-predicate-v2.py')
...
def check(case, observed):
    ...
    if cid in ('lifecycle-retained-sibling-growth', 'lifecycle-replaced-capture-descendant', 'lifecycle-replaced-chain-descendant'):
        ...
        require(all(r['result'] == {'kind':'Ok','value':[0]} for b in selected for r in requests(b)), 'V3-stable-request-results-zero')
        if index == 1:
    ...
    if cid == 'lifecycle-request-chain-points':
        ...
        for b in first + second:
            ...
            require(all(r['chain_id'] == b['resources']['entry_chain_id'] and r['result'] == {'kind':'Ok','value':[0]} for r in requests(b)), 'V3-sibling-request-chain-and-zero-result')
