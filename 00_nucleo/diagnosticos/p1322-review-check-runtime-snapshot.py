...
def transversal_review():
    r = preflight()
    data = read('p1322-transversal.json')
    frozen = read('p1322-transversal-freeze.json')
    corpus = read('p1322-transversal-cases.json')
    cases = {c['id']: c for c in corpus['matrix']['cases']}
    r.require(data['freeze_sha256'] == digest(DIAG/'p1322-transversal-freeze.json'), 'TRANSVERSAL_FREEZE', data['freeze_sha256'])
    ...
    for group in data['matrix']:
        ...
        for row in group['results']:
            ...
            for role, side in [('oracle', 'vanilla'), ('crystalline', 'crystalline')]:
                ...
                if item.get('exit_code') not in (0, 1) or any(k not in item for k in ('stdout', 'stderr')):
...
def main():
    ...
    parser.add_argument('mode', choices=['preflight', 'runtime', 'gates', 'supplement', 'transversal'])
    parser.add_argument('--output')
    args = parser.parse_args()
    started = utc()
    t0 = time.monotonic()
    r, extra = {'preflight': lambda: (preflight(), {}), 'runtime': runtime_review,
                'gates': gate_review, 'supplement': supplement_review, 'transversal': transversal_review}[args.mode]()
