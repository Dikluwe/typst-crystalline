# P1285 — receipt de verificação independente final v2

**Estado:** `FINAL_VERIFICATION_V2_COMPLETE`  
**Veredito binário:** `Not refined`  
**Regime:** protocolo completo de materialização segregada.  
**Papel:** novo verificador independente; não participou da autoria da intenção,
contrato, oráculos, testes, implementação, mutações ou da primeira verificação.  
**Janela da verificação:** `2026-08-30T17:00:18-03:00` a
`2026-08-30T17:05:06-03:00`.  
**Receipt escrito e hasheado:** `2026-08-30T17:08:33-03:00`.  
...
gates reproduzidos abaixo. Não alega equivalência funcional geral. O papel
verificador não corrigiu nenhuma falha e escreveu somente este receipt e
`00_nucleo/diagnosticos/typst-passo-1285-relatorio-v2.md`.

## 1. Autoridade, capacidades e limites

Entradas lidas: `AGENTS.md`, a skill `tekt-materializacao-segregada`, as duas
referências obrigatórias, exclusivamente o Passo 1285 autorizado, os artefatos
P1285 refinados, os nove L0, os nove consumers e a primeira verificação
histórica. Escrita limitada aos dois diagnósticos v2 autorizados. Não foram
lidos outros ficheiros de `00_nucleo/materialization/` nem quaisquer ficheiros
de `00_nucleo/context/`. Nenhuma falha foi corrigida.
...
## 2. Entradas refinadas verificadas antes e depois dos gates
...
| adversarial receipt v2 | `249ecd54e57a09496ec8c306d12c1fc04130bef42240ca090b8ebcc70a814f5e` |
| mutation plan v6 | `24a5c942b45dcccb4a03860f10e55e4358e6bbad81913d86abbc75565fa4f740` |
...
O binário executado pelo gate black-box tinha SHA-256
`1dda9c9b11453401b05738659b4c5de45db0a9923aac2d8d628895042e601088`.

## 3. Pins L0 e nove consumers refinados

O pin normativo é o SHA-256 após remover somente a linha
`Hash do Código:`. Os nove pins normativos coincidem com o contrato.
...
O full SHA atual de `eval/math.md` difere do full SHA congelado inicial
`7cdf5a1c0f93d1f58cda7f93eaae09eb0cbdcead3ca78864919569755c3321b2`,
mas o pin normativo é byte-idêntico. Pela política explícita da seção 2.2 do
contrato, isto é drift somente de selo e não drift semântico.

| Consumer | SHA-256 v6 exigido e observado antes/depois |
|---|---|
...
`crystalline-lint .` confirmou a linhagem/ownership sem violações de nível
error. As entradas protegidas e os nove consumers permaneceram estáveis durante
os gates.

## 4. Gate adversarial v2 auditado

A auditoria mecânica do receipt v2 encontrou:
...
A partição coincide com o plano v6:
...
também baseline idêntico antes/depois de `18/18` testes GREEN, restauração por
backup após cada mutante, invalidação dos artefatos incrementais sem mudança de
bytes e os nove hashes restaurados. Todos os nove hashes declarados no receipt
coincidem com os consumers observados nesta verificação.

## 5. Doze gates reproduzidos

| # | Comando | Janela | Exit | Contagem/resultado |
|---:|---|---|---:|---|
| 1 | `python3 lab/surface-inventory/run_p1285_oracles.py --typst target/debug/typst --baseline lab/surface-inventory/p1285-oracle-baseline.json` | `17:01:22–17:01:40` | 0 | `Preserved=42`, `Violated=0`, `Unknown=0` |
| 2 | `cargo test -p typst-shell p1285 -- --nocapture` | `17:01:50–17:01:54` | 0 | 5 passed, 0 failed |
| 3 | `cargo test -p typst-core p1285 -- --nocapture` | `17:01:58–17:01:58` | 0 | 9 passed, 0 failed |
| 4 | `cargo test -p typst-infra p1285_query_elements -- --nocapture` | `17:02:04–17:02:11` | 0 | 2 passed, 0 failed; integração filtrada 0/0 |
| 5 | `cargo test -p typst-wiring --test cli p1285_query_stdin_transporta_yaml_ate_stdout -- --nocapture` | `17:02:17–17:02:19` | 0 | 1 passed, 0 failed |
| 6 | `cargo test -p typst-core compiler::math:: --lib` | `17:02:26–17:02:26` | 0 | 336 passed, 0 failed |
| 7 | `cargo test -p typst-core repr_value_complex_types -- --nocapture` | `17:02:33–17:02:33` | 0 | 1 passed, 0 failed |
| 8 | `cargo build` | `17:02:36–17:02:37` | 0 | build concluído; warnings não fatais |
| 9 | `cargo test` | `17:02:47–17:03:07` | **101** | **FAILED**; execução parou em `typst-shell` |
| 10 | `cargo fmt --all -- --check` | `17:03:19–17:03:20` | 0 | árvore formatada |
| 11 | `git diff --check` | `17:03:26–17:03:26` | 0 | nenhum erro de whitespace |
| 12 | `crystalline-lint .` | `17:03:31–17:03:32` | 0 | 0 errors, 209 warnings, 1054 notes; 1263 resultados |

Uma repetição compacta da suíte integral, entre `17:04:57` e `17:05:06`,
preservou o mesmo exit subjacente `101` e permitiu contar exatamente as
unidades executadas antes da interrupção:

```text
typst-core                 5318 passed, 0 failed
typst-infra                 911 passed, 0 failed
p1250_svg_integrated          1 passed, 0 failed
typst-shell                  60 passed, 1 failed
total observado antes da interrupção = 6290 passed, 1 failed
```

Crates posteriores não foram executados porque o Cargo interrompeu a suíte no
primeiro binário de teste falhado. A repetição focal exata entre
`17:04:48–17:04:48` também terminou com exit `101`, `0 passed`, `1 failed` e
`60 filtered out`:

```text
cargo test -p typst-shell \
  p1225_eval_json_stroke_e_nominal_e_raw_permanece_proibido -- --nocapture
```

### 5.1 Falha decisiva da suíte integral

Teste:
`cli::tests::p1225_eval_json_stroke_e_nominal_e_raw_permanece_proibido`, em
`02_shell/src/cli.rs:1570-1605`.

```text
assertion failed:
serialize_eval(&Value::Color(Color::rgb(0, 0, 0)), EvalFormat::Json, false)
    .is_err()
```

A produção atual serializa `Value::Color` em JSON pela repr pública genérica.
O próprio L0 de CLI declara na seção P1285 que ela substitui, para P1285, as
restrições históricas que proibiam o fallback público por `repr`; C-JSON do
contrato inclui color entre os fallbacks públicos obrigatórios. Assim, a
expectativa final do teste P1225 ficou em conflito com o L0/contrato vigente.
O verificador não escolhe nem corrige o lado: a existência de um teste integral
vermelho basta para impedir `Refined`.

## 6. Primeira verificação preservada como histórico resolvido parcialmente

Os documentos da primeira rodada foram preservados sem sobrescrita:
...
Essa rodada emitiu `Not refined` por três condições:

1. `repr_value_complex_types` falhava em `gradient.linear()` versus
   `gradient(...)`;
2. `cargo fmt --check` falhava em `cli.rs`;
3. o oracle receipt ainda pinava o contrato antigo.

No ciclo refinado, as três condições históricas ficaram resolvidas e foram
revalidadas: o focal de repr passa `1/1`, `cargo fmt --all -- --check` passa e o
oracle receipt SHA `998a997…` pina o contrato final `647176…`. Isso não apaga
nem reescreve a rodada anterior. O novo teste integral vermelho descrito na
seção 5.1 é uma falha distinta e suficiente.

## 7. Proveniência exata da medição

Medição final pré-receipt: `2026-08-30T17:04:25-03:00`; HEAD
...
```text
 ...
 94 files changed, 645599 insertions(+), 1384 deletions(-)
```

Os dois diagnósticos v2 são untracked e, por definição, não aparecem em
...
Razão suficiente e independente: `cargo test` integral falhou com um teste em
`typst-shell`. Nenhum `Unknown` foi convertido em sucesso. Os oráculos `42/42`,
o gate adversarial combinado `30/30`, os focais verdes, o build, a formatação,
o whitespace e o lint arquitetural não autorizam `Refined` quando qualquer
gate final obrigatório permanece vermelho.
