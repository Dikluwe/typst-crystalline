...
def warning_run(c,phase,f,data,outroot):
    ...
    for side in roles:
        output=outroot/'warning'/f"{phase}-{side}-{c['id']}.{c['format']}";output.parent.mkdir(exist_ok=True)
    ...
    return row
def run(focal=False):
    f=json.loads((D/'p1335-transversal-freeze-r3.json').read_text());m.verify(f);data=json.loads((D/'p1335-transversal-cases.json').read_text());before=r.state();at=r.now();tmp=Path(data['temp']);rows=[];locations=[];extra=[];warnings=[]
    outroot=tmp/('r3-focal' if focal else 'r3-full');outroot.mkdir(exist_ok=False)
    ...
    m.verify(f);r.save('transversal-focal-r3' if focal else 'transversal-r3',dict(at=at,end=r.now(),manifest_sha256=r.sha(D/'p1335-manifest.json'),freeze_sha256=r.sha(D/'p1335-transversal-freeze-r3.json'),before=before,after=r.state(),binaries=f['binaries'],matrix=rows,location=locations,extra=extra,warnings=warnings,temp=str(tmp),artifacts={str(p):r.sha(p) for p in outroot.rglob('*') if p.is_file()},output_root=str(outroot)))
