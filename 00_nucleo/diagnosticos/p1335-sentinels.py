...
FIX=D/'p1335-fixtures'/'sentinels'
...
def freeze():
    ...
    focal=FIX.parent/'abs-origins.typ';subprocess.run(['cp',str(D/'p1334-ab-fixtures/origins.typ'),str(focal)],check=True)
    ...
    for row in required['cases']:
        c=dict(row);local=FIX.parent/'recent'/c['historical_step'];local.mkdir(parents=True,exist_ok=True)
        for name,body in c.get('fixtures',{}).items():
            path=local/name
    ...
    names=['p1335-sentinels-cases.json','p1335-sentinels.py','p1335-record.py','p1335-matrix.py','p1309-matrix.py','p1309-record-r2.py','p1307-r4-oracle.py','p1322-sentinels-cases.json','p1334-ab-cli-baseline.json','p1334-ab-cross-baseline.json','p1335-classification-required-sentinels.json']
...
def observe(c,profile,side,binary,phase,tmp):
    ...
    if c['route']=='eval':
        argv += [c['expression']]
        if c['format']=='json':argv += ['--format','json']
...
def run(phase,ids=None):
    f=json.loads((D/'p1335-sentinels-freeze.json').read_text());m.verify(f);corpus=json.loads((D/'p1335-sentinels-cases.json').read_text());before=r.state();start=r.now()
    ...
    m.verify(f);r.save('sentinels-'+phase,dict(at=start,end=r.now(),manifest_sha256=r.sha(D/'p1335-manifest.json'),freeze_sha256=r.sha(D/'p1335-sentinels-freeze.json'),before=before,after=r.state(),binaries=f['binaries'],rows=rows,counts=dict(Counter(x['runtime_class'] for x in rows))))
