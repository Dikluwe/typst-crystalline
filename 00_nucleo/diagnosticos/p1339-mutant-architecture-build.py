"""Independent M12 build with a pure reference parent and dedicated target."""
import base64, datetime, hashlib, json, pathlib, shutil, subprocess
root = pathlib.Path(__file__).resolve().parents[2]
area = pathlib.Path('/tmp/p1339-mutants.oPfqdA')
source = area/'architecture-negative'
target = area/'target-architecture'
output = root/'00_nucleo/diagnosticos/p1339-mutant-build-architecture-pure-r1.json'
...
record = {'authority_manifest_sha256':'842d6526739014022c073800148a47b3c886831e2198ab65bbfdbe73ce59411b','start':utc(),'argv':argv,'cwd':str(root),'source_root':str(source),'source_sha256':{str(p.relative_to(source)):sha(p) for p in files},'runner_sha256':sha(__file__)}
...
if p.returncode == 0:
    binary=area/'binaries/typst-architecture-pure-r1'
