"""P1338 immutable evidence and protection of the inherited worktree."""
import base64, hashlib, importlib.util, json, os, subprocess, sys, time
...
TARGET = '/tmp/p1338-target.vlNAmp'
def load(name):
...
def read(name): return json.loads((D / ('p1338-' + name + '.json')).read_text())
def save(name, data):
    path = D / ('p1338-' + name + '.json'); assert not path.exists(), path
...
def init():
    current = state(); previous = json.loads((D / 'p1337-closure.json').read_text())
    assert all(current[k] == previous['state'][k] for k in ['head', 'branch', 'diff', 'staged', 'product_inventory'])
    history = {str(p): sha(p) for p in D.rglob('*') if p.is_file() and not p.relative_to(D).parts[0].startswith('p1338-') and '__pycache__' not in p.parts}
    retained = {**previous['retained_prior_artifacts'], **previous['retained_mutant_artifacts'], **previous['predecessor_active_exports_preserved']}
    ...
    save('baseline', dict(at=now(), state=current, historical_preserved=history, retained_prior_artifacts=retained,
        previous_closure_sha256=sha(D / 'p1337-closure.json'), previous_review_sha256=sha(D / 'p1337-review-postclosure.json'), target=TARGET,
        snapshots={p: (ROOT / p).read_text() for p in [L0, SOURCE]},
        predecessor=dict(path='/tmp/p1337-target.F7jPNj/release/typst', sha256=sha('/tmp/p1337-target.F7jPNj/release/typst')),
...
def measure():
    ...
    expressions = ['(1,2).nope', '().absent', '{let início=(1,2);\n início.ausência}',
        '(1,2).len', '().first', '(1,2).last', '(1,2).len()', 'array.len((1,2))',
        '(1,2).first()', '(1,2).at(1)', '(1,2).nope(panic("arg"))', 'array.nope',
        'true.nope', 'none.nope', 'auto.nope', '"abc".nope', '(1pt).nope']
    ...
    save('measurement', dict(at=now(), baseline_sha256=sha(D / 'p1338-baseline.json'), before=before, after=after, rows=rows))
    for row in rows:
...
def command(name, argv):
    ...
    manifest = D / ('p1338-manifest-r1.json' if (D / 'p1338-manifest-r1.json').exists() else 'p1338-manifest.json')
    if manifest.exists(): data.update(manifest_path=str(manifest), manifest_sha256=sha(manifest))
