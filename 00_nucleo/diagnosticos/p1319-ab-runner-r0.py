...
def prepare(a):
    ...
    for mode in ('array','dictionary'):
        ...
        add('new-cross-file-rooted-'+mode,f'{{import "sub/paths.typ": rooted; csv(rooted, row-type: {mode})}}')
        ...
        add('new-cross-file-string-'+mode,f'{{import "sub/paths.typ": string; csv(string, row-type: {mode})}}')
    controls={
...
def freeze(a):
    ...
    files=[Path(__file__),CASES,Path(a.measurement),D/'p1318-ab-runner.py',D/'p1318-ab-cases.json',D/'p1318-ab-candidate-runs.json',D/'p1318-ab-freeze.json']
