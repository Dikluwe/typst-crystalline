...
- `00_nucleo/prompts/engine/layout/link.md` — layout calcula bbox acumulada e emite `FrameItem::Link { url, items, pos, size }`.
...
- `01_core/src/engine/layout/link.rs` → `f34dd97a`
...
### B.2 — Layout (`01_core/src/engine/layout/link.rs`)
...
- `01_core/src/engine/layout/helpers.rs` (`translate_frame_item`)
- `01_core/src/engine/layout/slicing.rs` (`rebase_item_y`)
- `01_core/src/engine/layout/cursor.rs` (dois locais de translate)
- `01_core/src/engine/math/layout/mod.rs` (translate math)
...
- `01_core/src/engine/layout/link.rs`: `p424_layout_link_tem_bbox_positiva`.
- `01_core/src/engine/eval/tests.rs`: `p422_link_body_texto_preserva_url` e `p422_link_body_implicito_url` agora validam `pos`/`size`.
...
**L0**: `engine/layout/link.md` e `infra/export/builder.md` atualizados; hashes sincronizados.

**Implementação**:
- `entities/layout_types.rs`: `FrameItem::Link` ganhou `pos: Point` e `size: Size`.
- `engine/layout/link.rs`: cálculo de bbox acumulada via `FontMetrics`.
- `engine/layout/helpers.rs`, `slicing.rs`, `cursor.rs`, `math/layout/mod.rs`: preservação/translação de `pos`/`size`.
