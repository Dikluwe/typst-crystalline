...
- `@prompt` em `text.rs` aponta para `text.md` (prompt integrado com secção
  `smallcaps`).
...
- `Content::SmallCaps { body: Box<Content> }` adicionado em `entities/content.rs`
  (linha ~618) com ctor `Content::smallcaps`, `is_empty`, `plain_text`,
...
- Walks de introspecção (`rules/introspect.rs` e
  ...
  container morfológico não-locatable.
...
- Suite completa: `cargo test --workspace -- --skip p350c_flag_on_nao_convergente_classifica` verde.

### 10.4 — Lint

- `crystalline-lint`: 0 erros, 0 drift. Resta apenas warning pré-existente
  `show-regex.md` órfão (não introduzido neste passo).

### 10.5 — Notas

- Não foi criado `smallcaps.md` separado; o prompt foi integrado em `text.md`
  para evitar prompt órfão no linter e manter a coesão do cluster de funções
  de texto.
- Consumer real continua scope-out (DEBT-53); o stub preserva honestidade
  epistémica — não há fallback software incorreto.
