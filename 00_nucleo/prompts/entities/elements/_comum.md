...
O layout precisa de `&mut Layouter` (contexto de `rules/layout`). Pô-lo no trait
...
onde está** em `rules/` — `rules/layout/mod.rs` (Divider, Heading) e
...
`rules/layout/elements/` é um **segundo eixo de atomicidade**, separável e
**fora do piloto** (minimiza o toque agora). Como a spec de comportamento de
layout não muda, **`rules/layout.md` NÃO é editado nem fatiado neste passo**
