...
## Proveniência `TextItem` nas métricas

`TextStyle.math_text_item` separa a morfologia de texto inline matemático da
rota de glifo sem desligar `style.math`.

Nas implementações `FontBookMetrics` e `FallbackFontMetrics`:

- a correção itálica de glifo-base singular só integra o avanço quando
  `style.math && !style.math_text_item`;
- `ssty_level_of` devolve `None` quando `math_text_item=true`;
- família, fallback math, variações, kerning, cap-height, ink bounds e tamanho
  permanecem governados pelos campos existentes;
- `AdvanceWidthKey` inclui a proveniência quando ela altera o avanço;
- `char_italics_correction` e `italics_correction` continuam a expor a
  métrica da fonte sem exceção nominal.

Quando `math_text_item=false`, MathIdent, MathText, números, GlyphFragment e
formas extensíveis preservam IC e `.st/.sts` conforme seus contratos. O
default `false` mantém callers não matemáticos e rotas de glifo inalterados.

Não inferir proveniência por texto, família, caractere ou fixture e não alterar
o trait `FontMetrics`.

## Aceitação

TextItem não recebe IC nem substituição `ssty` próprias de GlyphFragment;
fontes, fallback e demais métricas permanecem iguais. Rotas de glifo e número
continuam a receber as métricas matemáticas correspondentes.
