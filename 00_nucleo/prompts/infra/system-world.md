...
Hash do Código: 90acaa1c

**Camada**: L3
**Ficheiro alvo**: `03_infra/src/world.rs`
**Atualizado em**: 2026-07-31 (P935 — coverage eager, remove lazy cache e preload)
...
impl World for SystemWorld {
    ...
    fn candidates_for_char(&self, c: char) -> Vec<usize>; // delega a FontBook::candidates_for_char
...
## Cobertura Unicode — P935

**P880/P927 revogados.** O `FontBook` agora contém `FontInfo` com `coverage`
previamente preenchida por `font_info_from_bytes` durante a descoberta de
fontes (padrão vanilla). `SystemWorld` não precisa de cache lazy nem de
pré-carregamento condicional.

```rust
fn candidates_for_char(&self, c: char) -> Vec<usize> {
    self.font_book.candidates_for_char(c).collect()
}
```

O `FontBook::candidates_for_char` devolve os índices cujo bitmap contém o
bloco de 256 codepoints a que `c` pertence. O shaper continua a confirmar a
cobertura exacta com `face.glyph_index(c)` antes de usar o candidato.

### Campo removido

```rust
// coverage_cache: Mutex<HashMap<usize, Coverage>>,  // removido em P935
```

### API removida

```rust
// preload_coverage_if_needed  // removido em P935
// embedded_coverage_union     // removido em P935
// source_text_nodes           // removido em P935
```
...
| 2026-07-31 | P935 — coverage eager, remove `coverage_cache`, `preload_coverage_if_needed` e `embedded_coverage_union` | `system-world.md`, `03_infra/src/world.rs` |
