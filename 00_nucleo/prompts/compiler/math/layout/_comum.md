...
## P1293 — normalização de separador estrutural de `binom` no layout

## P1293 — despacho e reconstrução do carrier de `MathAttach`

### Medição anterior à decisão

O inventário read-only mede três consumos neste owner:
`01_core/src/compiler/math/layout/mod.rs:667-675` projeta os seis slots para
`layout_attach`; `:2029-2037` reconstrói ao aplicar default matemático; e
`:2242-2250` reconstrói ao aplicar estilo. Todos usam `Option::as_ref/map` e,
portanto, não conseguem conservar três estados. O recibo causal SHA-256
`80a9543c9450f2350a42fa48df2e42cac63109bd074ebb37c2ec424cba473d5c`
fecha a necessidade sem novo payload global.

### Decisão

O despacho passa ao layouter conteúdo somente para `Present(content)` e passa
separadamente a presença semântica do carrier que o owner `attach.md` consome;
`Omitted` e `ExplicitNone` não criam MathBox. As duas transformações mapeiam
recursivamente apenas `Present`, reconstruindo `Present(mapped)` e preservando
literalmente os outros estados. Nenhum caminho pode normalizar por
`Content::Empty`, largura ou tinta.

Esta mudança não altera `MathLayouter`, fase, defaults, parser, `Content`
global nem regras fora de MathAttach. O carrier público foi confirmado pelo
humano sob ADR-0127 categoria 1; o match continua estático, exaustivo e magro.

### Medição anterior à decisão

Proveniência: `HEAD 7dd25ff0e222b6c7c640d6bc7957b98f94227507`, branch
`Tekt`, working tree compartilhada e não commitada, instante
`2026-09-01T18:45:35-03:00`; `git status --short` SHA-256
...
O recibo público parcial P1293, SHA-256
`d41baadf3bc57a712d6906e27ce00b49797be4ebe5fb3fe0c65f9950714fbf3d`,
mede antes de qualquer correção geométrica:

- `binom` display com três lowers: vanilla `46.797666667 × 24.057`,
  candidato `54.101666667 × 24.057`;
- `binom` inline qualificado: vanilla `30.3325 × 7.8815`, candidato
  `39.6187 × 7.8738`;
- repr bilateral já convergente para
  `binom(upper: [n], lower: ([k], [j], [m]))`.

O mesmo recibo localiza o caminho em
`compiler/math/layout/mod.rs:648-660,1396-1445`: o denominator é um
`MathSequence` e cada separador estrutural `MathText(", ")` é medido como
texto integral **e** participa da classificação/gaps. `frac.rs:51-64` só
empilha as caixas e toma a largura já recebida, portanto não é o primeiro
owner causal. A medição vanilla P1293, SHA-256
`39f11f324677885ba093178fd5bc9cc40187a6dcceb67fa28fd55b247531c9a7`,
fixa a ordem e a morfologia dos múltiplos lowers; a fonte ratificada confirma
que `resolve_vertical_frac_like` insere um único símbolo vírgula entre lowers
(`lab/typst-original/crates/typst-library/src/math/ir/resolve.rs:738-747`,
SHA-256 `115d775641509a755a1b7f4bd6da26cc8502a09e0e23e303d38d4a7cd76e0112`),
deixando o avanço entre itens ao run matemático.

Classificação ADR-0107: ordem, pontuação, morfologia dos lowers e avanço
horizontal são observáveis da linguagem; a igualdade de hashes SVG serviu
somente para identificar as sondas. Inferência causal: o excesso é a dupla
materialização do mesmo limite — espaço textual terminal mais gap de classe.
Refutador: se retirar essa duplicação na vista transitória não fizer os dois
vetores convergirem, ou alterar texto literal/markup atômico não-separador, a
inferência está refutada e o implementador deve parar, sem ampliar heurísticas
ou owners.

### Decisão

`layout_sequence`, neste consumer proprietário, constrói uma vista transitória
para layout sem reescrever o `Content`. Quando uma folha matemática
`MathText` contém um separador pontuacional seguido somente por whitespace e
há um próximo irmão material, a normalização separa a pontuação visível da
fronteira de spacing:

1. a pontuação permanece um item material, na mesma ordem e com a mesma
   morfologia pública;
2. o whitespace terminal não é medido outra vez como avanço de texto;
3. a fronteira recebe exatamente uma contribuição de spacing pela tabela
   canônica de classes já consumida por `compute_gaps`, derivada do estilo e
   da fonte ativos;
4. texto literal, `Content::Sequence` textual atômica, whitespace terminal
   sem próximo irmão material e folhas sem a forma estrutural acima mantêm o
   caminho vigente.

A regra depende da categoria estrutural `MathText` + pontuação + whitespace +
irmão material, nunca do nome `binom`, da origem sintática, de coordenada,
largura ou constante de fixture. Não exige alteração de `spacing.rs`: a
normalização entrega à tabela existente uma única fronteira canônica. O
payload `MathFrac(line=false)`/`MathSequence` e o repr continuam byte-
conceitualmente intactos; nenhum campo, entidade, método público, default ou
fase é criado.

### Aceitação e ownership

- múltiplos lowers conservam quantidade, ordem, vírgulas e repr, com avanço
  display/inline equivalente ao vanilla no nível da linguagem;
- a altura e a fração sem linha permanecem sob `frac.md`; delimitadores sob
  seus owners vigentes;
- sequências de texto/markup não-separadoras e regras P1132s/P1132t não
  regressam;
- owner 1:1: este prompt legitima somente
  `01_core/src/compiler/math/layout/mod.rs`; `spacing.rs` não entra na
  allowlist sem nova medição e reabertura.

ADR-0127: correção interna de paridade, em fluxo contínuo após resselo; sem
novo gate humano. O gate funcional continua RED→GREEN bilateral sobre todos
os oito B-P07, com `Unknown` sempre bloqueante.

## P1293 — preservar a morfologia `TextItem` ao medir a correção itálica
...
Depois de isolar a cauda do wrapper semântico, os probes qualificados mantêm
um residual de `+0,1760pt` para `math.attach([x], tl:[L])` e `+0,2233pt`
para `math.attach([x], tr:[R])`. São exatamente as correções itálicas da
fonte: `16du` para `x` a 11pt e `29du` para `R` em Script a 7,7pt. O backend
vigente soma IC a qualquer texto de um glifo-base em modo matemático
(`03_infra/src/font_metrics.rs:1511-1537`).

A fonte vanilla separa a morfologia antes do layout: `resolve_text` converte
conteúdo textual não numérico em `TextItem`
(`lab/typst-original/crates/typst-library/src/math/ir/resolve.rs:271-305`),
que segue o layout inline/hbox (`typst-layout/src/math/text.rs:15-40`), sem o
`update_glyph`; já `GlyphFragment::update_glyph` conserva IC no avanço do
glifo não extensível (`typst-layout/src/math/fragment/glyph.rs:207-215`).

No cristalino, `layout_node` distingue diretamente `Content::MathIdent` e
`Content::MathText` (`01_core/src/compiler/math/layout/mod.rs:630-658`) e
ainda recebe o discriminante `Content::Text` no catch-all antes de produzir o
`FrameItem::Text` (`:887-903`). `FontMetrics` já expõe
`char_italics_correction` (`compiler/layout/metrics.rs:166-171`), com as
implementações vigentes em `infra/font_metrics.rs:638-650,1761-1780`.
Portanto a proveniência `TextItem` pode ser consumida neste owner, antes de ser
apagada, sem novo payload, trait, entidade, campo público, default ou fase.

Classificação ADR-0107: `TextItem` versus `GlyphFragment` é morfologia da
linguagem que decide o avanço; o discriminante Rust é somente o meio interno
de preservar essa diferença. Inferência: o residual é exclusivamente a IC
adicionada ao `Content::Text`. Refutadores: se subtrair exatamente o termo IC
já incluído não eliminar os deltas medidos, se alterar uma folha
`MathIdent`/`MathText`, ou se exigir transportar nova proveniência para além de
`layout_node`, a inferência falha e o implementador deve parar.

### Decisão

Na projeção direta de `Content::Text` em contexto matemático:

1. manter o layout inline/textual vigente, o mesmo texto, estilo, tinta,
   ascent, descent e `FrameItem`;
2. quando o texto tiver exatamente um glifo-base segundo a mesma fronteira já
   usada pelo avanço vigente, retirar da largura do `MathBox` somente
   `FontMetrics::char_italics_correction` do caractere e estilo ativos, isto
   é, neutralizar exatamente o termo que `advance` adicionou para a
   morfologia errada;
3. correção ausente/zero e texto verdadeiramente multi-glifo permanecem
   inalterados; a largura nunca é construída por constante, caractere
   especial, nome `attach` ou coordenada de fixture;
4. `Content::MathIdent`, `Content::MathText`, `GlyphFragment`, números e
   formas extensíveis conservam integralmente a IC exigida pelos seus caminhos
   vigentes. As obrigações P1132w e P975 não são relaxadas.

O owner de métricas permanece inalterado: `infra/font_metrics.md →
03_infra/src/font_metrics.rs` continua a fornecer avanço e IC canônicos para
glifos matemáticos; não deve ganhar exceção nominal ou por origem. A escolha
morfológica pertence a `_comum.md → compiler/math/layout/mod.rs`, onde
`Content::Text` ainda é observável.

Aceitação: as formas qualificadas de attach com folhas `[x]`, `[T]`, `[L]`,
`[R]` etc. usam avanço de `TextItem` equivalente ao vanilla, enquanto as
formas sintáticas com folhas matemáticas preservam avanço/IC de glifo. A
correção deve fechar os resíduos `0,1760pt` e `0,2233pt` sem regressar
MathIdent/MathText, P1132w, scripts ou formas extensíveis; todos os vetores
P1293 permanecem sob `Unknown = 0`.

Ownership 1:1: este prompt continua legitimando somente
`01_core/src/compiler/math/layout/mod.rs`; `font_metrics.rs`, `attach.rs` e
outros consumers ficam fora desta autoria. ADR-0127: correção interna de
paridade em fluxo contínuo após resselo e novo gate discriminatório. Se a
implementação exigir trait, `FrameItem`, campo público, payload, default ou
mudança de fase, deve parar para novo gate humano.

## P1293 — reabertura: conservar `TextItem` no estilo emitido até a extensão final

### Medição anterior à decisão

Proveniência do refutador de implementação: recibo B SHA-256
`8141334dfbb81315818e2926c2749dc23124b51fd359f7130e2a8d680d9dd6f4`,
medido em `2026-09-01T20:28:31-03:00` sobre HEAD
`7dd25ff0e222b6c7c640d6bc7957b98f94227507`, branch `Tekt`, working tree
compartilhada e não commitada (`git status --short` SHA-256
`27397d0a889680a07c6d9006aa05954af0c97e823bb6f49bfb7e66121129a697`;
diff restrito aos dois consumers então autorizados SHA-256
`887876bf66ea5a0705427830b892d5196312f724b5f56f7fdf4ca37536197dd5`).
O recibo causal independente original permanece SHA-256
`123a7c5b58e04a8701cb50f0c9224dd4b63ddc61c6120759391db30456277386`.

A primeira correção P1293 acima neutralizou IC apenas na largura intermediária
do `MathBox`. O resultado próprio ficou GREEN e o vetor display fechou, mas o
vetor attach inline qualificado permaneceu `19,8154pt` contra `19,7681pt` do
vanilla, residual `+0,0473pt`; os outros sete B-P07 ficaram GREEN. O recibo B
localiza a remedição posterior:
...
Nesse ponto o discriminante `Content::Text` já foi apagado, e o mesmo estilo
`math=true` emitido no `FrameItem` volta a ativar o avanço de glifo matemático
e a IC. Portanto a hipótese anterior — "basta ajustar a largura do MathBox e
manter o mesmo estilo do FrameItem" — foi refutada. A distinção precisa
atravessar a emissão, mas não exige payload novo se o discriminante já
existente em `TextStyle` representar a via correta desde o início.

### Fonte vanilla auditada antes da decisão

No vanilla ratificado `a51e02804`:

1. `resolve_realized` despacha `TextElem` para `resolve_text`, separado de
   `SymbolElem`/`resolve_symbol`
   (`lab/typst-original/crates/typst-library/src/math/ir/resolve.rs:149-161`);
2. texto não numérico vira `TextItem`, conservando a cadeia de estilos e
   acrescentando apenas bounds/overhang locais; números seguem `NumberItem`
   (`resolve.rs:28-36,271-305`), enquanto símbolos viram `GlyphItem`
   (`:327-355`);
3. `TextItem` é morfologicamente `spaced`/Alphabetic
   (`math/ir/item.rs:881-904`) e `layout_text` o envia a
   `inline::layout_inline`, preservando `styles`, tamanho resolvido, locator e
   hbox (`typst-layout/src/math/text.rs:15-40`);
4. o shaper inline resolve `TextElem::size` e os demais estilos da cadeia
   (`typst-layout/src/inline/shaping.rs:784-815`) e só fixa um script quando o
   estilo textual o declara; caso contrário deixa `guess_segment_properties`
   decidir (`:954-991`);
5. `GlyphItem` usa `layout_glyph`/`GlyphFragment`
   (`math/text.rs:67-123`), e o shaper matemático força explicitamente script
   OpenType `math` (`math/shaping.rs:168-212`). `NumberItem` também constrói
   `GlyphFragment` por caractere (`math/text.rs:44-64`).

Hashes das fontes auditadas: `resolve.rs`
`115d775641509a755b19e0e23e303d38d4a7cd76e0112`, `item.rs`
`273dbf55cb714ad7e40aa13c4648293260eee8d2818001952fbf21f9a4180b3a`,
`math/text.rs`
`c913d5620f91e1c747cecd3e05283245c1e558f9db280a193cc69972941b15e8`,
`inline/shaping.rs`
`673ae33f6b1e4100e9522e252fa1dcd3f3b28726b26d7c1b3e1674d7b84fc68b`
e `math/shaping.rs`
`ac656e97fd662e2b4705b8b222adae3d7c00df11928bde4c9cfcfbd896afa713`.

Classificação ADR-0107: `TextItem` versus `GlyphItem` é a morfologia que
escolhe layout/shaping e avanço; o booleano interno de `TextStyle` é apenas o
meio já existente de conservar essa escolha. ADR-0108: a decisão abaixo é
posterior ao residual e às duas vias `file:line`. Inferência: emitir o texto
direto desde o início pela via textual resolve tanto a caixa quanto toda
remedição posterior. Refutadores: residual diferente de zero, mudança de
tamanho/cramped/estilo, perda de fonte math aplicável, alteração em
MathIdent/MathText/número/MathClassOverride, ou necessidade de campo/trait/
FrameItem novo. Qualquer refutador exige parada e nova medição.

### Decisão que substitui somente a estratégia de largura da secção anterior

Para `Content::Text` **direto** que corresponde a `TextItem`:

1. construir um `TextStyle` local a partir do estilo matemático ativo,
   preservando tamanho efetivo, `math_size`, `cramped`, fonte, peso, itálico,
   bold, fill, tracking, leading, idioma e todos os demais eixos existentes;
2. desativar somente o discriminador que seleciona a via de glifo matemático
   (`math=false`, ou forma interna estritamente equivalente) **antes** de
   medir e emitir o texto;
3. usar esse mesmo estilo local para largura, ink bounds, `MathBox` e
   `FrameItem::Text`. Assim qualquer remedição posterior, inclusive
   `line_content_right`, continua na via textual e não reintroduz IC;
4. não subtrair manualmente `char_italics_correction`: o avanço textual
   canônico já exclui o termo de `GlyphFragment`, e uma subtração adicional
   duplicaria a correção;
5. restringir a regra ao braço estrutural direto `Content::Text`.
   `MathIdent`, `MathText`, número, `GlyphFragment`, formas extensíveis e
   `MathClassOverride(Content::Text)` intencionalmente promovido a glifo
   mantêm `math=true` e todas as obrigações P975/P1132w.

O tamanho de script e cramped não são inferidos novamente: já estão
materializados no `style.size`/`math_size`/`cramped` copiados. A família/fonte
ativa também é preservada; `math=false` distingue a mecânica de shaping e
avanço, não troca silenciosamente os demais eixos do estilo. Não há lista de
caracteres, constante, coordenada ou fixture na produção.

### Aceitação, ownership e gate

- o attach inline qualificado converge de `19,8154pt` para o fingerprint
  vanilla `19,7681pt`, com altura `9,6041pt`, e o display permanece exato;
- os sete B-P07 já GREEN, morfologia/repr/diagnósticos/spans e as guardas
  P1132s/P1132t/P1132w permanecem GREEN;
- texto direto em Script/ScriptScript preserva tamanho e cramped, mas segue o
  shaper/avanço textual; MathIdent/MathText/número continuam via math;
- `Unknown` permanece bloqueante e deve ser zero antes de julgamento B.

Ownership 1:1: este prompt legitima somente
`01_core/src/compiler/math/layout/mod.rs`. `helpers.md`/`helpers.rs` ficam
congelados no reparo de referencial já materializado; `font_metrics` e
`attach` permanecem byte-idênticos e fora da allowlist. A solução usa somente
um campo existente no braço onde a morfologia ainda é conhecida, sem novo
trait, entidade, `FrameItem`, campo público, default ou fase. ADR-0127:
correção interna de paridade em fluxo contínuo após resselo e novo gate
discriminatório; se qualquer superfície pública ou fase for necessária, deve
parar para gate humano.

## P1293 — reabertura por colisão entre modo math e proveniência `TextItem` (PROPOSTO; gate humano)
...
`7dd25ff0e222b6c7c640d6bc7957b98f94227507`, aplicou integralmente a decisão
anterior: `Content::Text` direto preservou todos os eixos do estilo salvo
`math=false`, e a mesma cópia foi usada na medição e emissão. Os testes próprios
ficaram `29/29` GREEN e sete dos oito B-P07 permaneceram GREEN. Porém o attach
inline qualificado mediu `19,1521 × 9,6041` contra vanilla
`19,7681 × 9,6041`, défice `-0,6160pt`. A troca removeu `0,6633pt` do
candidato anterior (`19,8154 -> 19,1521`), não somente a IC bloqueante de
`0,0473pt`.

O recibo causal independente SHA-256
`123a7c5b58e04a8701cb50f0c9224dd4b63ddc61c6120759391db30456277386`
já havia medido que `Content::Text` corresponde a `TextItem`, enquanto
MathIdent/MathText correspondem a glifos math. A fonte ratificada confirma:
`resolve_text` cria `TextItem` não numérico com styles ativos
(`math/ir/resolve.rs:271-305`); `layout_text` usa o hbox inline preservando a
chain (`typst-layout/src/math/text.rs:15-40`); símbolos usam `GlyphItem`
(`resolve.rs:327-355`) e a IC é acrescentada por `GlyphFragment::update_glyph`
(`fragment/glyph.rs:207-215`). A diferença é morfológica, mas os dois caminhos
continuam dentro do contexto matemático e podem compartilhar família, tamanho,
math-size e demais eixos.

No cristalino, a proveniência ainda existe somente no braço direto de
`Content::Text`; depois da emissão, `FrameItem::Text` carrega apenas a
`TextStyle`. A extensão posterior remede o item por esse estilo
(`compiler/layout/metrics.rs:265-295`). `TextStyle.math`, por sua vez, ativa
simultaneamente a cadeia math e a IC de glifo singular
(`03_infra/src/font_metrics.rs:1424-1440,1511-1537`). A estratégia anterior
colidiu esses dois significados.

Alternativas sem contrato público foram auditadas e refutadas: não existe
carrier privado no `FrameItem`; uma tabela lateral não sobrevive
clones/transforms; sobrecarregar `math_script`/`cramped` corrompe eixos
independentes; alterar `FontMetrics::advance` ou `FrameItem` também muda
contrato público e tem blast radius maior. Inferência: o menor reparo é o campo
default-preserving `TextStyle.math_text_item` proposto pelo owner da entidade.
Refutadores: o bit modificar fonte/shaping, atingir qualquer glifo math, ou não
fechar exatamente o residual com `math=true` preservado. Diante de qualquer
refutador, parar; não acrescentar heurística nominal ou constante.

### Decisão proposta após confirmação humana

No braço estrutural direto `Content::Text`:

1. clonar integralmente o estilo matemático ativo, mantendo `math=true`;
2. definir somente `math_text_item=true` antes de medir e emitir;
3. usar a mesma cópia em largura, ink bounds, `MathBox` e
   `FrameItem::Text`, para que toda remedição posterior conserve a
   proveniência;
4. não subtrair IC manualmente e não desligar a cadeia/fonte/shaping math;
5. manter `math_text_item=false` em MathIdent, MathText, NumberItem,
   GlyphFragment, formas extensíveis e `MathClassOverride(Content::Text)`
   promovido a glifo.

Tamanho, `math_size`, cramped, fonte, peso, itálico, bold, fill, tracking,
leading, idioma, direção e variações permanecem idênticos. Não há lista de
caracteres, nome `attach`, coordenada ou constante de fixture.

Aceitação linguística: o oitavo B-P07 converge ao vanilla preservando os sete
GREEN; direct TextItem mantém a mesma fonte/shaping e só perde a IC de
GlyphFragment; todos os caminhos de glifo math, P975/P977/P1132w, scripts e
formas extensíveis permanecem GREEN; `Unknown = 0` antes de qualquer aprovação
de B.

Ownership 1:1: este prompt continua legitimando somente
`01_core/src/compiler/math/layout/mod.rs`. O novo campo pertence a
`entities/layout_types.md`; a regra de IC pertence a `infra/font_metrics.md`.
ADR-0127 categoria 1: a solução requer novo campo público em entidade e fica
**bloqueada antes de código** até confirmação humana explícita. O fluxo
contínuo declarado na secção anterior está revogado para esta reabertura.
