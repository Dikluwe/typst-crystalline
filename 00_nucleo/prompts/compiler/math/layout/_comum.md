...
## Construção comum de `binom` e `MathAttach`

`binom` normaliza seu separador estrutural sem transformar vírgula em
conteúdo visível e conserva a ordem do lower variádico. O layout usa
`MathFrac(line:false)` entre delimitadores extensíveis.

`MathAttach` é reconstruído a partir do carrier triestatal sem colapsar
`Omitted`, `ExplicitNone` e `Present(Content::Empty)`. Apenas
`Present` produz fragmento; conteúdo vazio presente continua disponível para
as regras de spacing.

## Morfologia `TextItem`

Ao emitir `Content::Text` direto dentro do contexto matemático, o layout
preserva `math=true` e marca `math_text_item=true`. O bit atravessa merges e
reconstruções de `TextStyle` até a medição final.

Essa proveniência impede aplicar correção itálica e `ssty` exclusivas de
GlyphFragment, mas não altera família, fallback, tamanho, cramped, variações,
idioma, direção ou outras features. MathIdent, MathText, números,
GlyphFragment e formas extensíveis mantêm `math_text_item=false`.

A decisão é tipada e estrutural: não inferir por texto, nome de fonte,
caractere ou fixture. Cache e remedição distinguem os estados quando o
observável difere.

## Aceitação

Sintaxe e funções qualificadas produzem a mesma morfologia. Attach preserva os
seis slots, spacing e posições; binom preserva upper/lower e delimitadores.
TextItem não recebe IC ou `.st/.sts`, enquanto rotas de glifo e número
continuam a recebê-las.
