...
## Slots triestatais de `MathAttach`

Os seis slots `t,b,tl,bl,tr,br` chegam como `MathAttachSlot`:

- `Omitted` e `ExplicitNone` não materializam fragmento;
- `Present(content)` materializa o conteúdo, inclusive conteúdo vazio;
- conteúdo vazio presente preserva a reserva de `SpaceAfterScript`;
- ausência e `none` continuam layout-equivalentes.

O layout não infere presença a partir de `Content::Empty` e não reconstrói o
carrier por heurística.

## Largura e correção itálica

A largura horizontal considera os seis slots e a base no mesmo referencial.
Pre-scripts à esquerda não podem ser contados duas vezes pelo wrapper
semântico. Post-scripts, gaps, shifts verticais e constantes MATH preservam as
fórmulas do owner.

A correção itálica de `br` pertence ao fragmento semântico da base. Ela é
aplicada apenas na condição já definida pelo fragmento, sem depender do nome do
caractere ou de constante da fixture. `TextItem` não recebe propriedades
exclusivas de `GlyphFragment`.

## Emissão e aceitação

O carrier é consumido uma vez, na ordem pública dos slots. `Present` vazio
mantém a diferença morfológica mesmo que não tenha tinta. O layout preserva
conteúdo, ordem, classes math, baseline e posições relativas.

Casos com nenhum slot, `none`, markup vazio, pre-scripts, post-scripts e
combinações dos seis slots devem permanecer coerentes com a linguagem.
