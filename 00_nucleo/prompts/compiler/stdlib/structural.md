...
- A native assente os bits `HEADING_SET_*` do `HeadingElem` (ver `entities/elements/heading.md` §P829): `LEVEL` quando `level` vem posicional int **ou** named; `OUTLINED` quando named `outlined:` presente. Constrói via `Content::heading_native` / `Content::heading_numbered_native` (variante com `numbering`). Isto alimenta os métodos `has`/`at`/`fields` (P829-B, `compiler/eval.md`).

**Limitações / scope-outs**: Nenhuma conhecida nesta camada para os parâmetros `outlined`/`bookmarked`. P829: `heading(numbering:)` continua a transportar o gate na chain (`Content::Styled`) — `has("numbering")` devolve false (o campo não é materializado no `HeadingElem`; scope-out registado em `compiler/eval.md` §P829-B).
...
espaçamento de parágrafo entre eles (ver `compiler/layout.md` §P864).
...
**Limitações / scope-outs**:
- Renderização CSL real em `compiler/layout/bib_csl.rs` — ver scope-out dedicado abaixo.
...
- Renderização CSL real em `compiler/layout/cite.rs`.
...
**Limitações / scope-outs**: Layout real de hiperligações (`FrameItem::Link`, bbox, links internos, sublinhado azul) — ver `00_nucleo/prompts/compiler/layout/link.md`.
...
|------|-----------|-------|
| **CSL styling** (`bibliography` / `cite`) | Renderização real de estilos CSL (IEEE, APA, etc.), locales, ficheiros `.csl` customizados, múltiplas bibliografias, `title` customizado. | Lógica vive em `rules/eval/bibliography.rs` e `compiler/layout/bib_csl.rs`; o struct armazena `style`/`locale`. |
...
foram extraídos para `01_core/src/compiler/stdlib/numbering.rs` no P847.
A especificação perene vive em `00_nucleo/prompts/compiler/stdlib/numbering.md`.
