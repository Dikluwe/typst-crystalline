...
- Colateral medido em P810 e isolado em P821 como causa separada:
  ...
  (`compiler/stdlib/state.rs`) não tinha braço para `Value::Type`. Corrigido:
  display de um tipo é o seu nome curto (`type(1)` → "int",
  `type("abc")` → "str`).

**Testes canónicos P821** (ver `compiler/eval/tests.rs`, `state.rs`,
...
`repr_content` (`compiler/eval/repr.rs`), mesmo padrão de `MathCancel`/
