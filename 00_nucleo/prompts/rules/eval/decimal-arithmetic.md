...
No dispatch centralizado `eval_binary_op` em `01_core/src/rules/eval/operators.rs`, adicionar braços `Value::Decimal` junto aos braços `Int`/`Float` existentes.
...
Adicionar em `01_core/src/rules/eval/tests.rs` (ou módulo de testes apropriado):
