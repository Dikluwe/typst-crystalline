...
**Origem**: Passo 471 — módulo `sym` com tabela de ~70 símbolos prioritários; registado no scope como `Value::Dict`. **P731**: passa a `Value::Module` (paridade vanilla — medido: `type(sym)` → `module`).
...
O vanilla expõe `sym` como módulo acessível via `sym.arrow`, `sym.alpha`, etc. O cristalino implementa um subset de ~70 símbolos prioritários como `Value::Module` no scope global (desde **P731**; era `Value::Dict`).
...
Apenas entradas sem `.` no nome ficam no scope do módulo. As compostas (`"eq.not"`) estão na `SYM_TABLE` para `sym_lookup` mas não no scope.
