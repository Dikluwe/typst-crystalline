...
**Prompts relacionados**: `entities/elements/shape.md`, `entities/geometry.md`, `rules/layout/block.md`, `rules/layout/shape.md` (implementação actual).
...
- Em `lab/typst-original/crates/typst-layout/src/rules.rs:765-803`, todas as primitivas de desenho do vanilla são realizadas como `BlockElem::single_layouter`.
...
- `Content::Block` em `01_core/src/rules/layout/block.rs` já implementa terminamento de linha (`flush_line()`), espaçamento `above`/`below` e tracking de blocos consecutivos (`block_chain_active`, `prev_block_below_pending`).
- `Content::Heading` em `01_core/src/rules/layout/heading.rs` e `Content::ListItem`/`EnumItem` também terminam a linha em curso antes de se posicionarem.
...
- Não foram encontrados testes deste tipo na base actual (`01_core/src/rules/layout/tests.rs`).
