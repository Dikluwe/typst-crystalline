# Prompt L0 — `engine/layout/bib_csl` — Renderização CSL via hayagriva

Hash do Código: d4262e58

**Camada**: L1 · **Alvo**: `01_core/src/engine/layout/bib_csl.rs`
**Origem**: P418 (XL) — Bibliography/Cite CSL real.
**ADRs**: ADR-0062 (autorização de `hayagriva` em L1), ADR-0107 (paridade linguagem),
ADR-0109 (atomização forma B — free function em `engine/layout/`).
...
| 2026-06-23 | P420 (M): adicionar resolução de `.csl` customizado via path. | `bib_csl.md`, `bib_csl.rs`, `rules/eval/bibliography.rs`, `engine/layout/mod.rs`, `rules/stdlib/structural.rs` |
