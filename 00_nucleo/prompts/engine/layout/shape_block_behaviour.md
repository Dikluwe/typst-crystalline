...
- `Content::Block` em `01_core/src/engine/layout/block.rs` já implementa terminamento de linha (`flush_line()`), espaçamento `above`/`below` e tracking de blocos consecutivos (`block_chain_active`, `prev_block_below_pending`).
- `Content::Heading` em `01_core/src/engine/layout/heading.rs` e `Content::ListItem`/`EnumItem` também terminam a linha em curso antes de se posicionarem.
...
- Não foram encontrados testes deste tipo na base actual (`01_core/src/engine/layout/tests.rs`).
