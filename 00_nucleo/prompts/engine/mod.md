...
Hash do Código: bea4946d

**Camada**: L1
**Ficheiro alvo**: `01_core/src/engine/mod.rs`
...
|--------|----------|-----------------|------------------|
| `eval` | `eval.rs` | Motor de avaliação da AST → `Module` | `prompts/engine/eval.md` |
| `layout` | `layout.rs` | Conversor `Content` → `Frame` (layout de páginas) | `prompts/engine/layout.md` |
| `lexer` | `lexer/mod.rs` + `scanner.rs` | Tokenizador — texto → `SyntaxKind` stream | `prompts/engine/lexer/mod.md` |
| `math` | `math/mod.rs` | Subsistema matemático (MathLayouter + symbols) | `prompts/engine/math/mod.md` |
| `parse` | `parse.rs` | Parser — tokens → AST (`SyntaxNode`) | `prompts/engine/parse.md` |
| `scopes` | `scopes.rs` | Gestão de escopos aninhados de bindings | `prompts/engine/scopes.md` |
| `stdlib` | `stdlib.rs` | Funções nativas intrínsecas da linguagem | `prompts/engine/stdlib.md` |
...
- **Sem re-exports**: o consumidor (L4 wiring ou outros módulos L1) importa
  directamente de `crate::engine::eval::eval`, `crate::engine::layout::Layouter`,
...
use crate::engine::eval::eval;             // ok
use crate::engine::layout::Layouter;       // ok
use crate::engine::math::layout::MathLayouter; // ok
use crate::engine::stdlib::make_calc_module;   // ok

// Isolamento
grep "03_infra" 01_core/src/engine/ = vazio   // L1 nunca toca L3
```
