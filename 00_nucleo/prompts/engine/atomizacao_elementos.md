...
> movido para `engine/layout/{block,boxed,stack,pad}.rs`. Suíte verde (content-preserving), lint 0/0. Os
...
### `layout/mod.rs` — `fn layout_content` (`01_core/src/engine/layout/mod.rs:527-2383`)
...
### `introspect.rs` — o walk (`01_core/src/engine/introspect.rs:156`)
...
lógica numa **free function** `pub(super) fn layout<M,S>(layouter, h)` em `engine/layout/heading.rs`
...
- **Opção A — elemento-dono (REJEITADA pela ADR-0109 corrigida no P377).** `impl HeadingElem { fn
  ...
  genéricos).
- **Opção B — arquivo de layout por-elemento.** `engine/layout/heading.rs` com
...
> `engine/layout/<elem>.rs` (`pub(super) fn layout<M,S>(lo: &mut Layouter<'_,M,S>, e: &XElem)`);
...
> **~635 linhas** → `engine/layout/{block,boxed,stack,pad}.rs`. Maior encolhimento imediato do
...
> `engine/layout/<elem>.rs`):
> - `Heading` (`:765`, 44 linhas) → `engine/layout/heading.rs` — lê `chain`/`introspector`/
>   `current_location`/`style`/`regions` (estado diferente dos containers; prova a forma em quem lê
>   o Introspector).
> - `Transform` (`:1013`, 49 linhas) → `engine/layout/transform.rs`.
> - `Shape` (`:980`, 33 linhas) → `engine/layout/shape.rs`.
> - `Columns` (`:1544`, 44 linhas) → `engine/layout/columns.rs`.
>
...
> - `Place` (`:1166`, 58 linhas) → **novo** `engine/layout/place.rs` — lê `cell_align`/
...
>   `engine/layout/decorations.rs` — free function recebe `&Content` e re-match interno (1.º arm
>   agrupado atomizado); lê `font_size_pt`/`style.fill`/`regions`/`decoration_lines_collector`.
> - `Image` (`:1115`, ~36 linhas) → **completa** `engine/layout/image.rs` (`pub(super) fn layout`
>   junto do helper `calculate_dimensions` já lá).
> - `Figure` (`:909`, ~31 linhas de cálculo de prefixo) → **completa** `engine/layout/figure.rs`
...
>
> **Listas** → `engine/layout/{list_item,enum_item,terms,term_item}.rs` (lê fluxo-texto: `regions`/
> `page_config`/`flush_line`/`font_size_pt`/`style`/`layout_content`; TermItem lê `chain`).
> **Tabelas/Grid** (cluster `layout_grid`) → `Grid` dobra em `engine/layout/grid.rs` (junto de
...
> **Fatia 2 (refs/citações + avulsos), commit próprio.** 9 arms inline → `engine/layout/<elem>.rs`:
...
>
> **Fatia Text (isolada), commit próprio.** `Text` (`@653`, ~82 linhas) → `engine/layout/text.rs`.
...
> `engine/layout/equation.rs::Layouter::layout_equation` (`:21`) — **convenção `impl Layouter`
...
### Mudança em `engine/layout/cite.rs`
...
### Mudança em `engine/layout/mod.rs` — campo `last_cited_key`
...
### Detecção em `engine/layout/cite.rs`
