# Prompt L0 — `engine/eval/operators` — hub dos operadores do eval
Hash do Código: 15fbdf20

**Camada**: L1
**Ficheiro alvo**: `01_core/src/engine/eval/operators/mod.rs` (dispatcher)
...
- **Short-circuit de `and`/`or`**: implementado no dispatcher central
  ...
  `Expr::Binary`; L0: `engine/eval.md`). Os braços `And`/`Or` de
...
- **Literal percentual**: o mapeamento `Unit::Percent → Value::Ratio` vive
  em `eval/mod.rs` (L0: `engine/eval.md`).
